...
### المتبقّي
1. **(ب-٢)** شاشة المدخلين (إضافة · تسكين · نقل · إنهاء خدمة). **التالي.**
2. **(ج)** شاشة التسجيل: محافظة ← مدخل ← **نتيجة شهر** تُعلَّم فيها أيام الغياب/الإجازة، والجمعة والعطلة مُعطَّلتان، وشريط «أيام العمل ٢٥». (حلّت محلّ شبكة «أسبوع × مدخلين» بعد كلام العميل.)
...
### الاختبارات
`php artisan test tests/Feature/DataEntry` — **٦٤ اختباراً** (`BranchScaffoldTest` ١٢ · `AttendanceStatusesTest` ١٥ · `WorkingDaysTest` ٢٠ · `OfficialHolidaysTest` ١٧).
...
**⚠️ النشر:** الأربعة كاملة — الهجرات (٣ سابقة + ٤ جديدة) + فرع في config + ٩ راوتات **ينادي السايدبار إحداها** + `npm run build` (منفَّذ محلياً، و`git add public/build` صراحةً).
