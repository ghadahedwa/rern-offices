...
### الاختبارات
`php artisan test tests/Feature/DataEntry` — **١٠٢ اختبار** (`BranchScaffoldTest` ١٢ · `AttendanceStatusesTest` ١٥ · `WorkingDaysTest` ٢٠ · `OfficialHolidaysTest` ١٧ · `OperatorsTest` ٣٨).
...
**⚠️ النشر:** الأربعة كاملة — الهجرات (٣ سابقة + ٤ جديدة) + فرع في config + ١١ راوتاً **ينادي السايدبار إحداها** + `npm run build` (منفَّذ محلياً، و`git add public/build` صراحةً).
