...
feedback/digital                     → Feedback\DigitalPlatforms (feedback.digital)
...
feedback-results/digital             → FeedbackResults\DigitalRatings    (feedback-results.digital)
feedback-results/digital/summary     → FeedbackResults\DigitalSummary    (feedback-results.digital-summary)
```
...
### بوابة «تقييم المنصات الرقمية» ✅ **المرحلة الأولى (2026-09-14)**
طلب العميل: فورم **ثالث مستقل** بجانب التقييم والمقترح. `Feedback\DigitalPlatforms` (`/feedback/digital`) بلون أزرق (`--digital`)، والأسئلة **القسم الثاني من استمارة الوزارة الورقية** (`documentation/11.jpeg` و`22.jpeg`) **بصياغتها العامية كما هي** (قرار المستخدمة).
- **المصدر الواحد `FeedbackDigitalRating`**: `QUESTIONS` (النوع · النص · الاختيارات) و`SHOWN_WHEN` (ترجمة عمود «انتقل إلى») و`visibleQuestions()`. المكوّن يعرض ويتحقق ويحفظ **ما يظهر وحده**، والنتائج لاحقاً تحسب مقاماتها من الدالة نفسها.
- **الأنواع**: الأكواد الرقمية (١،٢) = `radio` · الحروف (A،B) = `checkbox` (q209 · q212) · مربعات الترميز = نص حر **اختياري** (q204 · q210) · q205 مقياس ٠–١٠. **q201 وحده إجباري دائماً**، وكل اختيار أو درجة **إجباري حين يظهر**.
- **المفاتيح أرقام الأسئلة** (`q201`…) في الأعمدة والكود — مطابقة الورقة ودليل الترميز فورية، وأسئلة القسم الثالث (٣٠١…) تمشي على النظام نفسه.
- ⚠️ **NULL = «لم يُسأل» لا «لا»** — كل عمود بعد q201 nullable. نسبة «قابلتهم مشاكل» تُحسب على مَن سُئل (الحاجزين) وحده.
- ⚠️ **السؤال المخفي يُحفظ NULL مهما بقي في حالة الفورم** — مَن اختار «حجزت» وأجاب ثم رجع لـ«بدون حجز» اختفت الأسئلة من الشاشة لا من الخصائص.
- ⚠️ **ظهور السؤال يتطلب ظهور ما يعتمد عليه** (السلسلة كاملة) — إجابة q211 باقية من مسار تُرك لا تُظهر q212.
- ⚠️ **الصفر درجة صحيحة في q205** — لا فحص «فارغ» بالقيمة الحقيقية (`$value ?`)، وإلا حُفظ الصفر NULL.
- **الاختيار المتعدد** في `feedback_digital_choices` (رأي × سؤال × اختيار): العدّ بـGROUP BY واحد على المحرّكين، وسؤال متعدد جديد = صفوف لا هجرة.
- **الحماية نفسها** بنوع ثالث `FeedbackGate::TYPE_DIGITAL` (قفل ٧ أيام منفصل عن التقييم والمقترح). ⚠️ **`FeedbackGate::modelFor()` يرمي استثناءً للنوع المجهول** — الكود القديم `suggestion ? … : Rating` كان سيفحص تكرار المنصات على جدول التقييمات صامتاً.
- **روابط الفورمين الآخرين** في شاشتي الشكر والحجب: `otherForms()` في الـtrait (حلّت محلّ `otherFormUrl/otherFormLabel`) + القالب `includes/other-forms-alt`.
- أنماط `.pills` (radio) و`.chips` (checkbox) **انتقلت إلى `form-styles` المشترك** — كانت في قالبي التقييم والمقترح.
- **معلّق على العميل**: «انتقل إلى ٣٠١» = الإرسال حالياً، والقسم الثالث يُضاف لـ`QUESTIONS`/`SHOWN_WHEN` حين يصل · تكرار q207 مع «مدة الانتظار» في فورم التقييم · اختيار B في q212 (اسم التطبيق مكرّر من ٢٠٢ — ربما نسخ خاطئ في الورقة).
- **المرحلة الثانية ✅** (النتائج) — انظر «نتائج تقييم المنصات الرقمية» في موديول النتائج. البوابة نفسها **لا تحتاج `npm run build`** (قالبها بلا Vite)، أما شاشات النتائج فتحتاجه.
...
- **سبب مطابقة رفض التكرار** (2026-09-15، هجرة `2026_09_15_000003` بملء للصفوف القائمة): `FeedbackGate::duplicateMatch()` يرجع مع تاريخ السماح `matched_by` (المفاتيح التي طابقت) و`matched_id`/`matched_at` (الرأي السابق)، وتُحفظ على صفّ الرفض وتُعرض في الشاشة برابط لشاشة النوع مفلترة بالمقر. ⚠️ **الهوية على صفّ الرفض هي المكتوبة في المحاولة لا هوية الرأي السابق** — بلاغ من السيرفر: رفضٌ برقم وهاتف لا رأي لهما، والمطابقة كانت ببصمة الجهاز مع رأي بهوية أخرى.
- ⚠️ **الوصول بزرّ الانتقال (`resume=1`) لا يُسجَّل رفضاً** (`evaluateGate(logRejection: false)`) ويبقى الحجب — كانت كل ضغطة «قدّم اقتراحاً أيضاً» بعد الإرسال تُسجَّل محاولة. الحارس `tests/Feature/Feedback/RejectionMatchTest.php` (٤، كُسِرت حراساته الأربعة وسقطت).
...
- `tests/Feature/Feedback/DigitalPlatformsTest.php` (٢٧) — الكروت الثلاثة · المسارات الستة لـ`visibleQuestions` · حفظ المسارين · **عدم حفظ مسار تُرك** · إلزام الظاهر وحده · النص الاختياري · **الصفر درجة** · القوائم البيضاء · المقر العام · القفل (نفس الجهاز · **منفصل بين الأنواع** · المحذوف) · **النوع المجهول استثناء** · روابط الفورمين الآخرين · نقل الهوية. 📌 كُسِرت حراساته الثلاثة عشر عمداً وتأكّد سقوطها.
- `tests/Feature/Feedback/AnonymousFeedbackTest.php` (١٨) — الإرسال بلا هوية · **صفحتا الفورم تُصدران كوكي البصمة مع التحميل** (الباقي يحقن الكوكي فلا يكشف غيابه) · قفل البصمة (نفس الجهاز/جهاز آخر/انتهاء المدة/**مُعرَّف ثم مجهول**) · **حارس المفاتيح الفارغة** · الكوكي التالف · السقف اليومي (ولكل مقر، **وعلى المُعرَّف بهاتف مختلَق**) · فلتر الهوية وقائمته البيضاء · المجهول داخل المتوسطات ونسبته · مؤشرات التدقيق وحارسها. 📌 كُسِرت حراساته التسعة عمداً وتأكّد سقوطها.
...
### نتائج تقييم المنصات الرقمية ✅ (2026-09-14)
شاشتان تحت الفرع نفسه بصلاحياته نفسها (`feedback.view` · `export` · `delete`) ونطاق المحافظات نفسه — **بلا صلاحية جديدة**:
- **`DigitalRatings`** (`/feedback-results/digital`): قائمة بنمط التقييمات + فلتر **المسار** (حجز/بدون حجز، قائمة بيضاء `DigitalRatingsQuery::PATHS`) + بحث في النصين الحرّين (q204 · q210) بـ`ArabicText` + حذف جماعي وسلة. **صف التفاصيل يعرض الأسئلة التي سُئلها المواطن وحدها** (`askedQuestions()`) بنصّها كما رآه. **Excel وحده** — ثلاثة عشر عمود إجابة لا تدخل صفحة مطبوعة.
- **`DigitalSummary`** (`/feedback-results/digital/summary`) — **صفحة مستقلة** (قرار المستخدمة) لا قسم في اللوحة: مؤشرات · المحاور بترتيب الاستمارة · درجة المنصة (متوسط + توزيع ٠–١٠ + مقارنة المنصتين) · النصوص الحرة **قراءة وبحث فقط** (قرار المستخدمة: بلا تصنيف) · الاتجاه الشهري لنسبة الحجز · **مقارنة المقرات بنسبة الحجز** (قرار المستخدمة) بزرّ الأعلى/الأقل أولاً (`?order=` قائمة بيضاء). Excel بأوراق + PDF.
- **الحسابات كلها في `DigitalReport`** — الشاشة والملفّان يقرؤون منه.
- ⚠️ **مقام كل نسبة = مَن سُئل السؤال** عبر `FeedbackDigitalRating::scopeAsked()` **المشتقّ من `SHOWN_WHEN`** لا مكتوباً يدوياً في تقرير. «٥٪ قابلتهم مشاكل» من ألف رأي هي «٢٥٪» من مئتي حاجز. **وكل نسبة معروضة بمقامها** («من ١٢٠ ممن سُئلوا»).
- ⚠️ **الاختيار المتعدد مجموع نسبه قد يتجاوز ١٠٠٪** — مكتوب بجانبه في الشاشة والملفّين.
- ⚠️ **الصفر درجة** — المتوسط على `whereNotNull` لا على «أكبر من صفر».
- ⚠️ **ترتيب المقرات على النسبة الدقيقة** ثم عدد الآراء، والمقرات دون `min_ratings_for_ranking` **بعد المرتَّبة** في الاتجاهين.
- **ورقة «التوزيعات» في Excel طويلة لا عريضة** (سؤال · إجابة · عدد · مقام · نسبة) — الشكل الصالح لجدول محوري. وملف الآراء **خلية ما لم يُسأل فارغة** لا «—».
- **اللوحة الرئيسية**: كرت مختصر (عدد · نسبة الحجز · متوسط الدرجة) يودّي للملخص (`DashboardReport::digitalHeadline()`)، و**نسبة المجهول ومؤشرات التدقيق صارت تشمل آراء المنصات** — في الشاشة والـPDF والـExcel معاً.
- `DashboardReport::monthExpression()` صارت `public static` يقرؤها `DigitalReport`.
- 📌 **طبقتا احتياط لا يسقط اختبارٌ بحذفهما** (مُثبَت بالكسر): تتبّع سلسلة الشروط في `scopeAsked` (كل شرط حالي يذكر سلسلته صراحةً — هي لأسئلة القسم الثالث) · قصر عدّ الاختيارات على مَن سُئل (الاختيارات لا تُحفظ لسؤال مخفي أصلاً).
- ⚠️ **مفتاح `fr_ip` كان مكرَّراً في `home.php`** (أضفتُ نسخة ثانية في دفعة الهوية فغيّرت «عنوان الجهاز» في الشاشات القديمة) — مؤشرات التدقيق صارت `fr_ip_line`. **افحص التكرار بعد أي إضافة مفاتيح.**
...
### الاختبارات
`php artisan test tests/Feature/FeedbackResults` — **١١٢ اختباراً** + `DigitalResultsTest.php` (٢١: الدخول والنطاق · المسار والبحث · التفاصيل · السلة والحذف النهائي مع الاختيارات · **المقامات** · المتعدد · الصفر · ترتيب المقرات بالاتجاهين · الاتجاه الشهري · فلتر الهوية · ملفّا Excel · حارس التصدير · توليد PDF · **عرض الملخص ببياناته** · اللوحة الرئيسية؛ ١٥ كسراً سقطت من ١٧ والاثنان طبقتا احتياط) (`BulkDeleteTest.php` ٢٠ للحذف الجماعي، `ExportTest.php` ٢٠ للتصدير، `PrintBarTest.php` ٤ لرسم أشرطة الـPDF، `RatingsPdfLayoutTest.php` ٦ لتخطيط جدول التقييمات المطبوع، `ScopeTest.php` ٢٨ للصلاحيات ونطاق المحافظات).
...
## موديول العاملين بالتعاقد ✅ **منجز (أ → هـ)**

**المرجع الواحد للبناء: [`documentation/contractors-system.md`](documentation/contractors-system.md)** — كل قرار وقاعدة وأثر كسرها مكتوب فيه.

طلب عميل (2026-09-04، ومراجعته 2026-09-05): الشهر العقاري متعاقد مع شركات توفّر **عاملين** موزّعين على المقرات. المطلوب موديول **مستقل عن المقرات**: المفتش يسجّل أسماءهم وهواتفهم لكل مقر، ثم يتابع **حضورهم وغيابهم وإجازاتهم** (يعرفها بمكالمة مع رئيس الفرع يومياً أو أسبوعياً)، ويطلع تقارير أسبوعية/شهرية/سنوية للفرد أو المقر أو المحافظة أو الجمهورية.

### إعادة التسمية «مدخلو البيانات» ← «العاملون بالتعاقد» (2026-09-06)
الموديول يضمّ **مدخل بيانات · مترجم · عامل · سائق · مساحي**، فالاسم القديم أضيق منه. الصفة الجامعة **طريقة التعاقد** لا المهنة، ولذلك اسم الفرع «العاملون بالتعاقد» والبادئة في الكود `contractors.*`.
- **الجداول أُعيدت تسميتها لا نُسخت** (هجرة `2026_09_06_000001`): `RENAME TABLE` عمليةٌ على البنية لا على الصفوف، والمفاتيح الأجنبية تتبعها. والنسخ إلى جداول جديدة كان يترك **توأماً بائتاً** في الداتابيز يبدو حيّاً وقد بطل تحديثه — والنسخ الاحتياطي مكانه خارجها.
- ⚠️ **أسماء الصلاحيات تتغيّر ومعرّفاتها لا**، و`role_has_permissions` يربط بالمعرّف — فالأدوار تحتفظ بصلاحياتها. ولذلك **تمسح الهجرة كاش Spatie في آخرها**: بدونه تبقى الأسماء القديمة في الكاش فيصير أصحابها بلا صلاحية (٤٠٣).
- ⚠️ **الهجرات الأقدم تبقى بأسمائها القديمة** — هي سجلّ تاريخي: داتابيز جديدة (الاختبارات) تُنشئ `data_entry_*` ثم تمرّ من هنا. وتعديلها يجعل هجرة إعادة التسمية تبحث عن جدولٍ غير موجود.
- ⚠️ **الهجرة تعدّ الصفوف قبل وبعد وترمي استثناءً إن اختلف الرقم.** وقد جُرِّبت على **نسخة السيرفر الحقيقية** (٢٧٦ عاملاً · ٢٧٩ تسكيناً): تطابقت الأعداد **ومجاميع المعرّفات** (٣٨٩٩٦ · ٣٩٨٤٥ · ٥٠٩٥٤ للمقرات) ذهاباً وإياباً مع `migrate:rollback`.
- ⚠️ **`attendable_type` يخزّن اسم الكلاس** — كان `attendance_days` فارغاً وقت التسمية (شاشة التسجيل لم تُبنَ)، ولذلك كان ذلك **أرخص وقت ممكن** للتسمية.
- 📌 والدرس التقني: `strtr` يطابق **الأطول عند الموضع** لا حسب ترتيب الخريطة، فـ`App\Support\DataEntryScope` صار `ContractorsScope`؛ وهو **حسّاس لحالة الأحرف**، فـ`forOperator` لم يطابق `scopeForOperator`. الاثنان سقطا في الاختبارات لا في المراجعة.

### صفة العامل (جدول `professions`)
قائمة مرجعية تحت **«إدارة النظام»** (`App\Livewire\Contractors\Professions\*` · راوتات `professions.*` · صلاحية `contractors.settings` نفسها، بلا صلاحية جديدة)، مزروعة بالخمس، و«مدخل بيانات» **`is_system`**.
- ⚠️ **الصفة على الشخص لا على تسكينه** (قرار العميلة): تقريرٌ عن شهرٍ مضى يعرض صفته الحالية. ولو لزم تأريخها فمكانها عمودٌ على `contractor_assignments`، وحينها لكل مدة تسكينٍ صفتُها.
- ⚠️ **الأساسية لا تُحذف ولا تُعطَّل، والمستعمَلة لا تُحذف** — والحُرّاس الثلاثة **في الإجراء لا في القالب**.
- **الهجرة عبّأت الـ٢٧٦ القائمين «مدخل بيانات»** — هم فعلاً كذلك، فلا صفّ بلا صفة.
- المعطَّلة تختفي من فورم الإضافة وتبقى على أصحابها، **وتظهر في فورم التعديل لصاحبها وحده** — وإلا بدا الحقل فارغاً فغيّرت الصفةَ حفظةٌ عابرة.

### الاستيراد بعد إضافة الصفة
- ⚠️ **الأعمدة تُقرأ بأسماء رؤوسها لا بمواضعها**: إضافة «الصفة» أزاحت المقر من `C` إلى `D`، و**قالبٌ قديم مملوء عند المفتش كان سيصير كلّ صفوفه «المقر مطلوب»** بلا سبب ظاهر له. والقراءة بالرأس تقبل القالبين معاً (والرؤوس البديلة في `ContractorsImport::HEADERS`).
- **القالب الجديد ينزل وعمود الصفة معبّأ «مدخل بيانات»** في صفوفه كلها — أغلب المتعاقدين كذلك، فالمفتش يغيّرها في الاستثناء وحده.
- ⚠️ **ولذلك يُقاس الصفّ الفارغ بالاسم والهاتف والمقر لا بالصفة** — وإلا صارت صفوف القالب غير المستعمَلة **ثلاثمئة خطأ «الاسم مطلوب»** في ملفٍ سليم.
- **الصفة الفارغة تُقرأ «مدخل بيانات»** (الملفات الموزَّعة سلفاً ترجع بالتصميم القديم بلا عمود صفة)، و**المعاينة تعرض عمود الصفة لكل صفّ** فالافتراضي مرئيّ لا صامت. و**المجهولة صفُّ خطأ** لا تجاهلٌ صامت ولا صفةٌ تُخترع.
- ⚠️ **الافتراضية تُعرَف بـ`is_system` لا بأول الترتيب** (في القارئ والقالب معاً): الترتيب حقلٌ يعدّله المدير من شاشة الصفات، فلو صار «مترجم» أوّلَه لصارت ملفات المفتشين تُستورد مترجمين بلا أثرٍ ظاهر.
- ⚠️ **الصفة تُعاد قراءتها من الجدول قبل الحفظ** كالمقر — صفةٌ عُطِّلت بين المعاينة والحفظ لا تُكتب.
- **قائمة الصفات في نطاق مسمّى ثانٍ** (`ProfessionList`) في الورقة المخفية، العمود `B` — والعمود `C` محجوز لبصمة المحافظة.
...
- ⚠️ **قرار «المفتش وحده يسجّل» أُلغي (2026-09-05)**: مَن مُنح `contractors.attendance` يسجّل — مفتشاً كان أو **حساب شركة** بصلاحيات `contractors.*` وحدها ومحافظاته من فورم المستخدم (بلا كود جديد). و**لا كيان «شركات»**: الأدمن ينشئ الحسابات ويحدّد نطاقها.
- البادئة **`contractors.*`** والفرع اسمه «العاملون بالتعاقد».
...
|---|---|---|
| `contractors.index` | قائمة العاملين + التقارير + دخول الفرع | |
| `contractors.attendance` | تسجيل الحضور | **مفصولة عن `edit` عمداً**: مَن يسجّل يومياً ليس بالضرورة مَن يعدّل الأسماء والهواتف |
| `contractors.create` / `.edit` / `.delete` | إدارة بيانات العاملين | لا تفتح الفرع وحدها |
| `contractors.export` | Excel + PDF | تحرس الملف لا الشاشة |
| `contractors.settings` | **حالات الحضور** (تحت «إدارة النظام») | بلا نطاق محافظات |
...
- ⚠️ و`contractors.settings` **مستثناة** من بادئة الفرع (`except`) لأنها تحت «إدارة النظام» — ولولا الاستثناء لطالبت مديرَ القوائم المرجعية بمحافظاتٍ لا معنى لها.
...
resources/views/livewire/contractors/{placeholder,statuses/*}.blade.php
```
راوتات الفرع `contractors.{index,attendance,reports}`، وشاشة الحالات `attendance-statuses.*` **داخل فرع إدارة النظام** (نمط الراوت مختلف عن نمط الفرع فلا يلتبسان).
...
contractors     name · phone · notes                  ← بلا مقر ولا محافظة ولا شركة
contractor_assignments   contractor_id · office_id · started_on · ended_on · end_reason
...
- ⚠️ **الحساب مقصور على مدة الخدمة** (من `contractor_assignments`): مَن التحق يوم ١٥ لا يُحسب حاضراً من يوم ١.
...
- ⚠️ **تداخل مدد التسكين لا يُعبَّر عنه في المحرّك** — حارسه `ContractorAssignment::overlapsExisting()` قبل كل حفظ.
- ⚠️ **CarbonImmutable**: حلقات الأيام تُكتب `$day = $day->addDay()` — الصيغة المعتادة تُنتج حلقة لا نهائية.
- ⚠️ **شاشة العطلات للسوبر أدمن وحده** (قرار العميل) — أضيق من `contractors.settings` عمداً: القائمة قومية، وعطلةٌ خاطئة تغيّر تقارير الجمهورية كلها.
...
`App\Livewire\OfficialHolidays\{Index,Create}` · راوتات `official-holidays.*` **خلف `role:super-admin`** داخل فرع إدارة النظام · بندها في السايدبار تحت «إعدادات العاملين بالتعاقد».
...
### شاشة العاملين ✅ **(ب-٢)**
`App\Livewire\Contractors\{Index,Create}` · راوتات `contractors.index` (القائمة) و`contractors.{create,edit}`.
- **النطاق في `App\Support\ContractorsScope`** — نقطة التوسيع الوحيدة (شاشة · منسدلة · تقرير · تسجيل · تصدير)، و**ثلاث حالات**: `null` بلا حدّ · `[]` لا يرى شيئاً · `[...]` محافظاته. النطاق عبر **مقر التسكين** لا عمود محافظة.
- ⚠️ **كل معرّف من العميل يُقرأ عبر النطاق** (`scopedContractor`/`allowsOffice`) لا بـ`findOrFail` — وإلا نُقل مدخلُ محافظةٍ أخرى بمعرّفٍ مدسوس. والمنسدلات مفلترة كالصفوف.
...
- **الفلاتر والترتيب على البنية المشتركة** (`x-filter-bar` + `WithTableSorting` + `WithPerPage`) — خمسة فلاتر في **شبكة من ثلاثة أعمدة** (قرار العميلة): بحث · محافظة · مقر في صفّ، ورأس العمل · **نوع المقر** في الذي يليه، وترتيب بقائمة بيضاء (الاسم · الهاتف · تاريخ الالتحاق كعمود محسوب).
- ⚠️ **فلترا المحافظة والنوع يضيّقان منسدلة المقرات** (`ContractorScope::officeOptions($gov, $type)`) **ويُصفّران المقر المختار** — وكلاهما يُهمَل في الاستعلام حين يُختار مقرٌّ بعينه (المقر أخصّ منهما).
- **الهاتف سطرٌ ثانٍ تحت الاسم في عمودٍ واحد** لا عموداً مستقلاً (بيانا تعريفٍ لشخصٍ واحد، والعمود المستقل كان يضيّق ما بعده). ولذلك زال رأسُ ترتيب الهاتف من الشاشة و**بقي `phone` في القائمة البيضاء** (`?sort=phone` من رابطٍ قديم ما زال صالحاً). ونِسَب الأعمدة الستة مجموعها ١٠٠٪.
- **الجدول خمسة أعمدة**: الهاتف سطرٌ تحت الاسم، و**تاريخ الالتحاق سطرٌ تحت المقر** (قرار العميلة 2026-09-06: هو تاريخ بدء التسكين في المقر لا بيانَ تعريفٍ بالشخص، فمكانه تحت المقر). وأزرار العمليات الأربعة **نصية ظاهرة بهيئة حبّات بلا حدود** (خلفية باهتة بلونها: **أزرق للتعديل · ذهبي للنقل وإعادة التسكين · رمادي لإنهاء الخدمة · أحمر للحذف**، ولون واحد لفعلين متجاورين يجعلهما كتلة واحدة في العين) — أربعة إطارات في خلية واحدة ضجيج بصري. وجُرِّبت قائمة «⋯» ورُفضت. ونِسَب الأعمدة مجموعها ١٠٠٪.
- ⚠️ **الحبّة الرمادية `zinc-200` — درجتان أغمق من خلفية الصفّ**، البيضاء والمؤرشَفة (`zinc-50`) معاً. بدرجةٍ واحدة (`zinc-100`) لا تبدو زرّاً أصلاً: بلّغت العميلة أن «تعديل» و«إنهاء الخدمة» **بلون الخلفية**.
- **الصفّ المؤرشَف بخلفية باهتة**، فلا يختلط بالعامل في فلتر «الكل».
- ⚠️ **`phone` و`started_on` باقيان في قائمة ترتيبٍ بيضاء بلا رأسٍ في الشاشة** (`?sort=` من رابطٍ قديم يظلّ صالحاً)، ولذلك يبقى العمود المحسوب `current_started_on` في `render()`.
- ⚠️ **الجدول الفارغ يفرّق بين «لا مدخلين بعد» و«الفلتر لم يطابق»**، و**الفرق يُقرأ من النطاق لا من `hasActiveFilters()`**: فلتر الحالة الافتراضي (على رأس العمل) يُخفي المؤرشَفين وهو ليس فلتراً مفعّلاً، فشاشةٌ فيها مؤرشَفون وحدهم كانت ستعرض «لا مدخلين» وزرَّ الإضافة. المؤشّر `hasAnyOperator` واستعلامه لا يقع إلا على صفحة فارغة.
...
- **منسدلات التسكين تعرض ما تُسكَّن به عمالة متعاقدة وحده** (`Office::scopeWithContractWorkers` ← `office_types.has_contract_workers`): استراحة · تحت الإنشاء · معلَّق بقرار · منتهي العمل به · غير مستغل · أراضٍ ليست مقارَّ عمل (١٧٦ مقراً من ١١٣٠). **خانة على النوع بنمط `is_public`** تُدار من شاشة أنواع المقرات — لا قائمةَ أسماءٍ في الكود: الأسماء تُكتب بإملاءات مختلفة ونوعٌ جديد يحتاج تعديلاً. والهجرة تطابق بالتطبيع.
- ⚠️ **الاستيراد مستثنى** (قرار العميلة): القالب والقارئ يقرآن `officeOptions()` لا `assignableOffices()` — تضييق القالب يجعل مقراً مكتوباً في ملفٍ موزَّع سلفاً «غير معروف».
- ⚠️ **والمقر الذي لا عمالة به يبقى في شاشة المقرات وتقاريره وعلى مَن سبق تسكينه فيه** — الحدّ للاختيار لا للعرض؛ إخفاؤه من السجلّ يُخفي تاريخاً صحيحاً.
- ⚠️ **النقل وإعادة التسكين مفتوحان على محافظات الجمهورية كلها** (قرار العميلة 2026-09-07) — والعامل يبقى مرئياً لناقله لأن له تسكيناً سابقاً في نطاقه. والحارس `allowsAssignment(withinScope: false)`، بينما **الإضافة تبقى داخل النطاق**.
- ⚠️ **مقرات المودالين لا تُحمَّل قبل اختيار محافظة**: القائمة مفتوحة على أكثر من ألف مقر، وتحميلها في كل عرضٍ للشاشة تكلفةٌ بلا فائدة — ومنسدلةٌ بألف خيار لا تُستعمل.
...
`App\Livewire\Contractors\Import` + `App\Support\Contractors\{ContractorsTemplate,ContractorsImport}` · راوت `contractors.import` بصلاحية `contractors.create`.
...
### شبكة تسجيل الحضور ✅ **(ج-١) 2026-09-15**
`App\Livewire\Contractors\Attendance` (اختيار وحراسة) + **`App\Support\Contractors\AttendanceSheet`** (المنطق كله: الأعمدة · الصفوف · البصمة · الحفظ) + `resources/js/attendance-grid.js` (`Alpine.data('attendanceGrid')`، مستورَد من `app.js`).
**محافظة ← مقر ← شهر**، صفٌّ لكل مَن خدم في المقر خلال الشهر × عمودٌ لكل يوم. الخلية تبدأ «حاضر» **بصبغة باهتة من لونها في جدول الحالات** (١٢٪ — طلب المستخدمة 2026-09-17؛ الأخضر المصمت جُرِّب بلقطة مقارنة ورُفض: يُغرق الغياب والإجازة ويخلط الأحمر بالأخضر لمن لا يميّزهما)، والتعليم بأداة (إجازة/غائب/… **بترتيب جدول الحالات**، و«إجازة» قبل «غائب» بهجرة `2026_09_17_000001` لا تُبدِّل إلا الترتيب المزروع كما هو) بالضغط أو السحب داخل الصفّ، والحفظ **دفعةً واحدة**. فلتر «العامل» داخلها بدل شاشة مستقلة، وبحثٌ بالاسم عابرٌ للمقارّ يفتح مقرّه وشهره. **استيراد الإكسيل (ج-٢) منجز بصيغةٍ مبسَّطة — انظر أدناه.**
- **«وصل الكشف» على المقر** (قرار العميلة): زرّ للمقر وخانة لكل صفّ، ويُخزَّن في **`attendance_reviews`** بمفتاح (العامل · المقر · الشهر) — المنقول في منتصف الشهر يصل كشفا نصفيه منفصلين. غير المعلَّم يخرج في التقارير «غير مراجَع» لا «حاضر».
- ⚠️ **`attendance_statuses.is_default`** يعلّم «حاضر» — لا تُخزَّن ولا تظهر أداةً (`scopeMarkable`)، لأن صفّاً بها يُحسب استثناءً فيُنقص الحضور. **عمودٌ لا مطابقة اسم** (الاسم قابل للتعديل)، **وليس «طبيعة الحالة»** المؤجَّلة.
- ⚠️ **الأيام المفتوحة = أيام العمل ∩ تسكينه في هذا المقر** (`WorkingDays::contractorCalendar(…, officeId:)`). يومٌ في مقرٍّ آخر مقفول، **ولا يُحذف غيابه** بحفظ هذه الشبكة — «الشبكة مصدر الحقيقة» محصورة بما تعرضه.
- ⚠️ **بصمة الكشف**: الحفظ يُرفض إن تغيّرت الحالة المخزَّنة بعد الفتح — الخلية الراجعة «حاضر» تحذف ما تحتها، فبلا البصمة يمحو المستخدمُ غيابَ زميلٍ سجّله في الأثناء.
- ⚠️ **ما يصل من العميل يُصفّى في `sanitizeMarks`**: يومٌ مفتوح لهذا الصفّ · حالةٌ من `markable` **أو الحالة المخزَّنة نفسها في الخلية** (المعطَّلة تبقى ولا تُكتب جديدة) · صفٌّ لم يُرسله العميل **لا يُمسّ** · عاملٌ ليس في الشبكة يُهمَل · المقر يُفحص بالنطاق عند الحفظ (٤٠٣).
- ⚠️ **الحذف والتعديل بمعرّف الصفّ لا بـ`where('date', 'Y-m-d')`** — العمود المصبوب `'date'` يُكتب `'Y-m-d 00:00:00'` على sqlite. و`month` في المراجعات بنطاق مفتوح لا مساواة.
- ⚠️ **الإعداد يُقرأ مرة من `data-config`** لا من وسيط `x-data`، و**مفتاح الشبكة = المقر · الشهر · `$version`** (يزيد بعد الحفظ و«تجاهل التغييرات») — لا البصمة: الـkeepalive وفلتر العامل يُعيدان العرض فلا تُمحى تغييرات لم تُحفظ.
- ⚠️ **`Alpine.data` يُسجَّل فوراً إن وُجد `window.Alpine`، وإلا عند `alpine:init`** (بلاغ السيرفر 2026-09-17: خلايا بيضاء لا تُعلَّم). **Livewire لا يُعيد تحميل الصفحة عند تغيّر اسم الأصل (هاش Vite)** — `data-navigate-track` يُعيد التحميل لتغيّر الـquery string وحده — فتبويبٌ مفتوح قبل النشر يحقن `app.js` الجديد بعد بدء Alpine و`alpine:init` مضى. **أي `Alpine.data` جديد في الحزمة يتبع النمط نفسه.** مُثبَت بإعادة إنتاج (بناء فارغ ← فتح صفحة ← بناء جديد ← wire:navigate) سقط قبل الإصلاح ونجح بعده.
- **الاختيارات مقفولة ما دام في الكشف تغييرات**، والخروج بـwire:navigate يُعترض (`livewire:navigate` قابل للإلغاء) بمودال، و`beforeunload` للمتصفح.
- **«وصل الكشف» في عمودٍ بجوار الاسم** لا في آخر الصفّ (يحتاج تمريراً فيُنسى)، والجدول **الاستثناء الوحيد من «يدخل عرض الشاشة»**: تمرير داخل البطاقة والاسم `sticky`. وحدود الخلايا **بيضاء عريضة** — بلونٍ قريب تذوب الأيام في كتلة. ونصّ الخلية **حرفان** («غا» · «إج»): «إ» منفردةً تُقرأ «!».
- منسدلة المقرات **ما خدم فيه عاملٌ خلال الشهر** وحده. و**«وصل الكشف» يمنع حذف العامل** (`Index::deleteRow`) كسجل الحضور.
- مُعايَنة فعلياً في Chrome على نسخة البيانات المحلية: التعليم بالسحب · شريط التغييرات · الحفظ وبقاؤه بعد إعادة التحميل · مودال الخروج — بلا أخطاء JS.


### كشف الشهر بالإكسيل ✅ **(ج-٢) 2026-09-17**
**`App\Support\Contractors\AttendanceMonthFile`** (بناء · قراءة · معاينة · حفظ) فوق `AttendanceSheet`، في **صفحةٍ مستقلة** `App\Livewire\Contractors\AttendanceFile` (`contractors.attendance-file` · `/contractors/attendance/file` · صلاحية `contractors.attendance` · بندٌ في منيو الفرع **بحارس `Route::has`**) — طلب العميلة 2026-09-17 فصلها عن الشبكة. زرٌّ في رأس الشبكة يفتحها **على المحافظة والشهر المعروضين** (`?gov=&month=`)، وبعد الحفظ رابط «افتح كشف الشهر على الشاشة». طلب العميلة: **«أسهل طريقة بلا تعقيد»** — فالملف **صورة الشبكة بقواعدها**:
- **ملفٌّ للمحافظة عن شهر** (قرار العميلة): صفٌّ لكل (عامل × مقر) × عمودٌ لكل يوم. **حرف الحالة** أول حرف من اسمها («غ» · «إ»، وحرفان إن تكرّر) · الفارغ حاضر · المقفول «-» ويُهمَل ما كُتب فيه. الهمزة والاسم الكامل مقبولان.
- **رفع الملف = وصل كشف كل صفٍّ صالحٍ فيه** (قرار العميلة) — والعامل الغائب عن الملف **لا يُمسّ ويبقى «غير مراجَع»**، فلا مانعَ لاختلاف قائمة العاملين ولا عمود «وصل الكشف» في الملف.
- ⚠️ **ملفُّ شهرٍ/محافظةٍ غير المعروضين يُرفض**، وكذلك ما ليس كشفاً من النظام (ورقة «بيانات» مخفية: إصدار · محافظة · شهر) أو تغيّرت أعمدة أيامه (تُطابَق بأرقامها في الرأس).
- ⚠️ **المطابقة بمعرّفَي العامل والمقر (عمودان مخفيان) ورقمٍ خالص `ctype_digit`** — «12x» بالتحويل المجرّد عاملٌ حقيقي. وصفٌّ أُضيف باليد خطأ: الملف لا يُنشئ عاملاً.
- ⚠️ **صفٌّ فيه حرفٌ مجهول يُرفض كاملاً** (لا نصف شهر)، والباقي يُحفظ.
- ⚠️ **الحفظ كلٌّ أو لا شيء عبر المقرات**: بصمة كل مقر عند المعاينة في خاصية `#[Locked]`، ومقرٌّ تغيّر يُلغي الحفظ كله (استثناءٌ داخل transaction). **والملف يُعاد قراءته عند الحفظ** — العلامات لا تعيش في حالة المكوّن.
- **القالب معبّأٌ بالمسجَّل** فرفعه لا يمحو، والخلية الراجعة فارغة **تحذف** — والمعاينة تعرض «سيُحذف» صريحاً.
- المعاينة تُصفَّر بتغيير المحافظة أو الشهر.
- **قائمة منسدلة في خانات الأيام** (`STOP`): المفتوحة بحروف الحالات المفعَّلة **ثم صور الألف الأخرى** («إ,غ,ا,أ,آ» — طلب العميلة: مَن يكتب بالكيبورد يكتب «ا»، وتحقق Excel حرفيّ بينما القارئ يطبّع)، و**المقفولة (جمعة · عطلة · خارج التسكين) لا تقبل إلا «-»** — بلاغ العميلة 2026-09-17: تحققٌ واحد على الكتلة كلها كان يقبل «غ» في الجمعة فيُهمَل صامتاً. النطاقات **رأسية متصلة لكل عمود** (شمال القاهرة ١٣٩ عاملاً = ٣٠ نطاقاً · ٢٧KB).
- **ملخّص كل صفّ بعد الأيام كالشبكة** (طلب العميلة): أيام العمل **رقمٌ ثابت** (حساب النظام) · الحاضر `= أيام العمل − (COUNTA − COUNTIF("-"))` · عددُ كل حالة `COUNTIF` بصور ألفها كلها — معادلاتٌ تتغيّر بالكتابة **ولا تُقرأ عند الرفع**.
- ⚠️ **المعاينة تُسمّي أيام العلامات المُهمَلة** («الأيام: ٤، ١٦») لا عددها وحده — عطلةٌ أُضيفت بعد تنزيل الملف تبدو فيه يوماً عادياً مفتوحاً.
- **الأيام المفتوحة أخضر «حاضر» الباهت** (لون الحالة ممزوجاً بالأبيض ١٢٪ — Excel بلا شفافية)، والحرف يغطّيه بتنسيقٍ شرطي بلون حالته.
- ⚠️ **اسم العامل والمقر يلتفّان** (أسماء المقرات حتى ١٣٦ حرفاً كانت تسيل فوق الأيام)، و**ارتفاع الصفّ يُقدَّر بعدد الأسطر** — Excel لا يعيد حسابه عند الفتح.
- ⚠️ **رفع ملفٍّ قديم يحذف ما سُجِّل بعد تنزيله** (الملف مصدر الحقيقة، ولا بصمة وقت التنزيل — أُسقطت للتبسيط). الحارس الوحيد «سيُحذف N» الأحمر في المعاينة. وقع فعلاً في المعاينة المحلية 2026-09-17: ملفٌّ نُزِّل قبل رفعٍ آخر حذف علاماته الثلاث.
- ⚠️ **النطاق**: المحافظة تُفحص في `scopedGovernorate()` للتنزيل والرفع معاً.
- **سقط من التصوّر الأول** (§٦ بند ٢-أ): حماية الورقة · المعادلات · تكرار عناوين الطباعة · تنبيه الشهر (صار رفضاً).
- مُعايَن في Chrome: تنزيل ← تعبئة ← رفع ← «سيُضاف ٣» ← حفظ ← الشبكة بعلاماتها و«وصل الكشف» لكل من في الملف.

### تقارير الحضور ✅ **(د) 2026-09-22**
**ثلاث شاشات منفصلة** (قرار العميلة) فوق محرّك واحد **`App\Support\Contractors\AttendanceReport`** — المصدر الواحد للأرقام تقرأ منه الشاشات والملفّ معاً. والطبقة المشتركة `Reports\Concerns\BuildsAttendanceReport` (المدى الحرّ · الحراسة · النطاق · التصدير).
```
contractors.reports.governorates → /contractors/reports/governorates  (وهو البيان العددي)
contractors.reports.office       → /contractors/reports/office
contractors.reports.contractor   → /contractors/reports/contractor
contractors.reports              → تحويلةٌ للأولى (الرابط القديم لا يكسر)
```
- **الوحدة الذرّية صفٌّ لكل (عامل × مقر)**، وكل تقريرٍ جمعٌ لها — فلا يتفرّع حسابٌ عن حساب.
- **المعادلة مغلقة**: `أيام العمل = حضر + غير مراجَع + (غاب + إجازة + …)` مع **تفكيكها في رأس التقرير** (`٣٠ − ٤ جُمَع − عطلة = ٢٦`). **عدٌّ لا نسبة.**
- ⚠️ **لا عمود «غير مراجَع»** (طلب المستخدمة 2026-09-22): المصطلح من اختراع الكود لا يعرفه مَن يقرأ التقرير، **والعمود يكون صفراً لكل الصفوف في الشهر العادي** فهو ضجيجٌ لا معلومة. فأيامه تدخل «حضر» (`AttendanceReport::attended()` — دالةٌ واحدة يقرؤها الثلاثة)، وتبقى المعادلة مقفولة.
- **وما بقي من الفرق تنبيهٌ يظهر حين يقع وحده**: «٣ عاملين لم تُرصد أيام حضورهم… وأيامهم محسوبة حضوراً» (`unrecordedContractors()` — **عددُ عاملين لا أيام**: الإجراء أن يسأل المفتش عن كشفٍ لم يصل). في الشاشة والـPDF معاً. بدونه تصير خانة «وصل الكشف» في الشبكة بلا أثرٍ في التقارير.
- **والغياب والإجازة المسجَّلان يبقيان كما هما** رُصد الكشف أو لم يُرصَد — واقعةٌ مكتوبة لا اشتقاق، فلا يبتلعها «حضر».
- ⚠️ **`scopedOfficeIds()` حارس تسرّب النطاق**: `applyToContractors` يُبقي العامل مرئياً بتسكينٍ واحدٍ داخل النطاق، **فصفوفه تحمل تسكيناته في محافظاتٍ ليست لي**. بدونه تظهر أيامُ مَن نُقل خارج نطاقي — **تسرّبٌ صامت لا يمسكه فلتر العاملين**.
- ⚠️ **اليوم الواحد لا يُعدّ مرتين** (أول تسكينٍ بالتاريخ يملكه)، و**الأيام تُوزَّع قبل فلتر المقر لا بعده**.
- **تقرير العامل يفصّل التواريخ** («غاب يوم ٢») ولا يعرض يوماً خرج من الحساب.
- ⚠️ **المحافظة إلزامية في تقرير المقر وحده** (طلب المستخدمة 2026-09-22): بلا تحديدٍ كان يجمع عاملي **كل مقرات النطاق** فيخرج جدولٌ بمئات الصفوف — وهو تقرير **مقر** لا تقرير جمهورية. والرفض **يُصفّر المعروض** لا يُبقيه (نتيجةٌ قديمة تحت محدداتٍ جديدة تُقرأ على أنها نتيجتها). و`search()` يُعاد تعريفه في المكوّن بـalias `runSearch`.
- **شبكة المحافظات كلها ظاهرة بلا تمرير** (طلب المستخدمة): الصندوق القصير بسكرول داخليّ كان يُخفي أغلبها، فلا يرى المستخدم ما اختاره ولا ما بقي. صارت شبكةً بخمسة أعمدة تملأ عرض الشريط + زرّ «مسح التحديد».
- **منسدلتا المقر والعامل فوقهما سطر بحث** (`x-contractors.searchable-select`): البحث **في السيرفر بـ`ArabicText`** لا في المتصفح — تكراره في JS يجعل ما يجده المستخدم هنا مخالفاً لبقية الشاشات. ⚠️ **والخيار المختار يبقى في القائمة ولو لم يطابق البحث** وإلا بدا أنه ضاع أو انتقل صامتاً، **والبحث يُمسح عند تغيير المحافظة** وإلا خرجت قائمةٌ فارغة بلا سببٍ ظاهر.
- ⚠️ **الحالة الفارغة تفرّق بين «لم تختر» و«لا بيانات»** — رسالةٌ واحدة لحالتين كانت تقول للمستخدم إن العامل بلا أيام وهو لم يختر عاملاً أصلاً.
- ⚠️ **ثلاثة أعطالٍ في الـxlsx كشفها فحص محتوى الملف** (لا نجاح التنزيل): **الصفر يُكتب خانةً فارغة** (`0 == null` — العلاج `WithStrictNullComparison`) · **الصفّ الفاصل الفارغ يُسقطه الكاتب فيزيح رأس الجدول عمّا يحسبه التنسيق** (القيم سليمة فلا يكشفه فحص البيانات — **الاختبار يقيس لون خلية الرأس**) · **الصفّ المجمَّع كان بلا اسم** فيطبع رقم المحافظة مكانها.
- **تقرير PDF** ✅ (2026-09-22): كنترولر واحد `Contractors\AttendanceReportPdfController` للمستويات الثلاثة (المستوى في الرابط بقائمةٍ بيضاء)، والفلاتر في الـ**query string** فالرابط قابل للمشاركة. **والأرقام من `AttendanceReportQuery` نفسه الذي تقرأ منه الشاشة** — كلاسٌ استُخرج في هذه الدفعة ليقرأ منه المكوّن والكنترولر معاً (الكنترولر خارج المكوّن، ولو بنى استعلامه لخالف الشاشة). سقف ٢٠٠٠ صفّ بتنبيه، والحارس `contractors.export` **داخل الكنترولر** (الرابط يُفتح منسوخاً).
- ⚠️ **`white-space: nowrap` على رأس الجدول المطبوع — مقيسٌ لا مقدَّر**: **اسم حالةٍ من كلمتين** («إجازة مرضية») في عمودٍ ضيّق يكسر **صفّ الرؤوس كله** سطرين (٢٨.٦٥pt بدل ١٨.٤٥)، والاسم يكتبه المدير من شاشة الحالات فلا يُقصَر من الكود. ومقيسٌ أن **تصغير الخط لا يمنعه** (٧.٥pt ← ٢٦.٢٥ · ٧pt ← ٢٥.٠٥) ولا تقليل الهوامش، **وأن `nowrap` لا يُخرج الجدول عن الصفحة** (٧٩٥.٨pt في الحالتين). ⚠️ **ولا تُعمَّم على تقارير رأي المواطن**: هناك الرأس المنكسر في عمودٍ رقميٍّ ضيّق و`nowrap` لم ينفع — الفرق في المحتوى لا في mpdf، **فيُقاس لكل جدول**.
- ⚠️ **الوضع (رأسي/أفقي) مقيسٌ أيضاً**: بحالتين (٧ أعمدة) يسع الرأسيُّ سطراً واحداً، وحالةٌ ثالثة فأكثر تحوّله أفقياً.
- 📌 **فرضيةٌ سقطت بالقياس**: ظننتُ أن «عدد العاملين» و«غير مراجَع» هما اللذان ينكسران فبنيتُ التقرير أفقياً دائماً — وقياسُ خمس صيغ للرؤوس أعطى ٢٨.٦٥pt لكلها، فتبيّن أن الكاسر اسمُ الحالة لا رؤوسي.

### لوحة الفرع ✅ **(هـ) 2026-09-23**
`App\Livewire\Contractors\Reports\BranchDashboard` (`contractors.dashboard`) + `App\Support\Contractors\DashboardReport`. **صفحة دخول الفرع** (قرار العميل)، و**بلا سجل نشاط المستخدمين** — وهو ما يميّزها عن لوحة المقرات.
- ⚠️ **الفلتر يسبق ما يفلتره**: وُضع أسفل الصفحة مرةً ليترك أول شاشةٍ للأرقام (طلب المستخدمة)، **فصار المستخدم يقرأ نتيجةً مفلترة ولا يرى ما فلترها** — فصحّحته هي نفسها. والحلّ أنه يبقى فوق **ويصغر**: شريطٌ في سطر واحد بلا عنوان قسمٍ ولا شبكةِ مربعات مفتوحة، والمحافظات في **منسدلةٍ تُظهر عدد المختار** فيبقى النطاق معلوماً بلا سطرٍ يشرحه.
📌 والدرس: عَرَضٌ عولج بسطرٍ إضافي («الأرقام معروضة لـ…») كان سببه **الترتيب** نفسه — فلمّا صحّ الترتيب زال السطر.
- **المحافظات مخطط أعمدة، والصفات بطاقات** (قرارا المستخدمة): المخطط لا خريطة، و**الصفات خمسٌ معدودة فالبطاقة تقول ما يقوله المخطط بلا قراءة محور** — بخلاف المحافظات التي يجعل عددُها المقارنةَ البصرية هي المعلومة. وحارسه يعدّ الـ`canvas` (واحدٌ لا اثنان): `assertSee` على الاسم لا يفرّق لأن المخطط يكتب أسماءه في الـHTML أيضاً، وأربع بطاقات (على رأس العمل · مقرات بها عاملون · محافظات · منتهية خدمتهم)، وحضور الفترة.
- **منتقي فترة كالتقارير** (قرار المستخدمة) — الطبقة المشتركة نفسها، لكن ⚠️ **اللوحة تعرض فوراً لا بعد ضغط زرّ**: `mount()` يستدعي `search()`، وكل تغيّر للمدى يعيد الحساب (`x-contractors.report-filters` بـ`:auto` تُخفي زرّ العرض وتجعل الحقلين `.live`).
- ⚠️ **أرقام الحضور مفوَّضة لـ`AttendanceReportQuery`** لا محسوبة في اللوحة — لوحةٌ تحسب حضورها بنفسها تخالف تقرير الفترة نفسها فيرى المستخدم رقمين لشيءٍ واحد. و**«على رأس العمل» = `Contractor::inService()`** حرفياً كشاشة العاملين.
- ⚠️ **«أيام العمل» كلمةٌ بوحدتين**: في التقارير أيامُ **عاملٍ واحد** (٢٦)، وعلى اللوحة **مجموع أيام كل العاملين** (٢٩٣٣٦ على السيرفر = ١٥٤٤ عاملاً × ١٩ يوماً). فالرقم الكبير **ليس بطاقةً مفردة** على اللوحة: العنوان **الغياب والإجازات** (أرقامٌ صغيرة تُقرأ كما هي)، والمجموع تحتها **مقاماً يشرح وحدته** («من أصل … يوم عمل لـ… عاملاً»). بلاغ المستخدمة 2026-09-23.
- ⚠️ **المحافظة بصفرٍ تبقى في المخطط** — «لا عاملين في بني سويف» ليس «بني سويف غير موجودة»، وحذفُها يُخفي الثغرة التي يبحث عنها المفتش. (والصفة الخالية تُسقَط: صفةٌ لا أحد عليها ليست ثغرة.)
- ⚠️ **المخطط بـ`wire:ignore` ومعه `wire:key` يحمل بصمة البيانات**: بلا `wire:ignore` يُتلِف Livewire المخطط في كل عرض، وبه وحده **يتجمّد على أرقام أول تحميل ولا يتبع منتقي الفترة** — رقمٌ قديم بجوار فلترٍ جديد يُقرأ على أنه نتيجته.
- 📌 **خريطة `entries` في `config/branches.php` هي «اسم الراوت => الصلاحية» لا العكس** — وتطابقُ الطرفين في الصيغة القديمة (`contractors.index => contractors.index`) كان يُخفي الاتجاه، فقلبُها أهبط صاحبَ العرض على شاشة الحضور. أمسكه `BranchScaffoldTest`.

### المتبقّي
1. سؤال مفتوح: أيام العمل من `offices.working_days` لو اختلف مقرٌّ عن مقر؟

### الاختبارات
`php artisan test tests/Feature/Contractors` — **٢٧٧ اختباراً**، منها `AttendanceReportTest` (١٩: المعادلة · «غير مراجَع» بمقرّه وشهره · بقاء الغياب المسجَّل · نسبة اليوم لمقرّه · **تداخل التسكين** · مدة الخدمة · العطلة المتأخرة · فلتر المقر · **التوزيع قبل الفلتر** · العاملون المتمايزون · أعمدة الحالات · المدى المقلوب؛ 📌 خمسة كسور سقطت) و`AttendanceReportScreensTest` (٣٠: الحراسة · النطاق و**تسرّب المنقول** · المقر المدسوس · تفصيل التواريخ · التصدير و**محتوى الـxlsx ولون رأسه**؛ 📌 كُسِرت حراساته وسقطت، **واثنان لم يسقطا أولاً فأُضيف لهما اختبارا «المنقول» الحقيقيان**)، منها `AttendanceMonthFileTest` (١٤: القالب · الحفظ ووصل الكشف · الهمزة · الحذف · الغائب عن الملف · الصفّ المجهول · المقفول · الشهر الآخر · ليس قالباً · بلا معرّف ومدسوس · **كلٌّ أو لا شيء بمقرّين** · **المعرّف التالف** · النطاق · التنزيل؛ 📌 كُسِرت حراساته العشرة وسقطت، واثنان منها لم يسقطا أولاً فقُوّي اختباراهما) و`AttendanceGridTest` (٢٤): العرض والأيام المقفولة · حصر المقر · الحفظ والحذف والتعديل · اليوم المقفول · غياب المقر السابق · الافتراضية والمجهولة · المعطَّلة · المدسوس · الصفّ غير المُرسَل · **البصمة** · وصل الكشف ومفتاح المقر · منع الحذف · النطاق (رابط · حفظ · بحث لمنقولٍ خارج النطاق) · منسدلة المقرات · الشهر التالف. 📌 **كُسِرت حراساته الأحد عشر عمداً وسقطت كلها** (وكشف الكسر أن اختبار البحث الأول لم يبلغ حارسه فأُضيف اختبار المنقول).
...
**⚠️ نشر شبكة الحضور:** ثلاث هجرات (`is_default` · `attendance_reviews` · ترتيب «إجازة» قبل «غائب») + `npm run build` (**أول JS في `app.js`** — بدونه الشبكة صفحةٌ بلا تفاعل) و`git add public/build resources/js` صراحةً + الأربعة.
