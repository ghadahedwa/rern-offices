...
- ⚠️ **فلترا المحافظة والنوع يضيّقان منسدلة المقرات** (`DataEntryScope::officeOptions($gov, $type)`) **ويُصفّران المقر المختار** — وكلاهما يُهمَل في الاستعلام حين يُختار مقرٌّ بعينه (المقر أخصّ منهما).
...
### الاختبارات
`php artisan test tests/Feature/DataEntry` — **١١٩ اختباراً** (`BranchScaffoldTest` ١٢ · `AttendanceStatusesTest` ١٥ · `WorkingDaysTest` ٢٠ · `OfficialHolidaysTest` ١٧ · `OperatorsTest` ٤٢ · `OperatorsImportTest` ١٣).
