...
### 4. لوحة التحكم — Dashboard ← **الأولوية: سجل النشاط أولاً**

#### ما يظهر لـ super-admin (يرى كل شيء)
- **KPI Cards**: إجمالي المقرات، عدد المحافظات، عدد المستخدمين، مقرات مضافة هذا الشهر
- **توزيع بياني**: توزيع المقرات على المحافظات، حسب النوع (`type_id`)، حسب الحالة الإنشائية
- **من متصل الآن**: من جدول `sessions`
- **سجل النشاط**: آخر 10 عمليات (إنشاء/تعديل/حذف مقر) مع المستخدم والوقت والتفاصيل
- **ملخص إحصائيات**: إجمالي معاملات التوثيق لآخر سنة (group `transactions`)

#### ما يظهر للمستخدم العادي (مقيّد بمحافظاته)
- عدد مقرات محافظاته فقط
- عدد المحافظات المسندة إليه
- آخر مقر تم تعديله في محافظاته
- **لا يرى**: إحصائيات المستخدمين، الجلسات النشطة، بيانات محافظات غيره

#### ترتيب التنفيذ
1. **سجل النشاط** ← **الأولوية الأولى**
   - الحزمة: `spatie/laravel-activitylog`
   - تسجّل: إنشاء/تعديل/حذف مقر (مع القيم القديمة والجديدة)، تسجيل دخول المستخدمين
   - عرض: جدول بأعمدة (الوقت، المستخدم، الإجراء، العنصر، التفاصيل) مع فلاتر بحث
2. **KPI Cards** (queries مباشرة، بسيطة التنفيذ)
3. **من متصل الآن** (من جدول `sessions`)
4. **التوزيع البياني** (charts)
