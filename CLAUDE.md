...
## موديول مدخلي البيانات 🔨 **قيد البناء — الصلاحيات وحالات الحضور منجزة**

طلب عميل (2026-09-04): الشهر العقاري متعاقد مع شركة توفّر **مدخلي بيانات** موزّعين على المقرات. المطلوب موديول **مستقل عن المقرات**: المفتش يسجّل أسماءهم وهواتفهم لكل مقر، ثم يتابع **حضورهم وغيابهم وإجازاتهم** (يعرفها بمكالمة مع رئيس الفرع يومياً أو أسبوعياً)، ويطلع تقارير أسبوعية/شهرية/سنوية للفرد أو المقر أو المحافظة أو الجمهورية.

### قرارات العميلة — لا تُراجَع بلا سؤالها
- **المفتش وحده يسجّل** → النطاق **محافظة** (نفس pivot `governorate_user`)، ولا عمود مقر على `users`.
...
### المتبقّي
1. **جدولا `data_entry_operators` و`attendance_days`** — المخطَّط: الحضور **polymorphic** (`attendable_type/id`) ليخدم موظفي «تشكيل المكتب» لاحقاً بلا هجرة مؤلمة، مع `UNIQUE(attendable, date)` و`recorded_by` (سجل مساءلة).
2. **شاشة التسجيل: شبكة أسبوع × مدخلين** بزر «الكل حاضر» ثم استثناء الغائب — المفتش يجمع أسبوعاً في مكالمة واحدة، فالإدخال صفّاً صفّاً لن يُستعمل.
3. التقارير الثلاثة + التصدير.
4. أسئلة مفتوحة للعميلة: أيام العمل تُقرأ من `offices.working_days`؟ · **اليوم غير المسجَّل ليس غياباً** (يحتاج تمييزاً في العرض) · انتقال المدخل بين المقرات · الشركة المتعاقدة واحدة أم أكثر؟

### الاختبارات
`php artisan test tests/Feature/DataEntry` — **٢٧ اختباراً** (`BranchScaffoldTest` ١٢ · `AttendanceStatusesTest` ١٥).
📌 كُسِرت حراساتها الست عمداً (حارس الأساسية · شرط التأكيد المسبق · تعطيل الأساسية · `except` · تطبيع البحث · حارس الصلاحية) وتأكّد سقوطها قبل الاعتماد.

**⚠️ النشر:** الأربعة كاملة — ٣ هجرات + فرع في config + ٦ راوتات **ينادي السايدبار إحداها**.
