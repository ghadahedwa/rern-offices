{{-- ── تاب المطالبات (محافظة + تاريخ + مبلغ) ── --}}
@php
...
<div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 space-y-4">
    ...
    <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.claims_tab_demands') }}</h3>
        </div>

        @if($this->canEdit())
        <button type="button" wire:click="openAddDemand"
                ...
                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                    ...
                    <td class="px-4 py-2.5">
                        <div class="flex items-center justify-center gap-1">
                            @if($this->canEdit())
                            <button type="button" wire:click="openEditDemand({{ $demand->id }})"
                            ...
                            <button type="button" wire:click="askDeleteDemand({{ $demand->id }})"
...
<div x-show="$wire.showDemand"
     x-transition.opacity
     @click.self="$wire.showDemand = false"
    ...
    <div x-transition:enter="transition ease-out duration-200"
        ...
        <div class="flex items-center justify-between px-5 py-3.5 bg-[#c9a847]">
            <div class="flex items-center gap-2.5">
                ...
                <h3 class="text-sm font-semibold text-white">
                    {{ $editingDemandId ? __('home.claims_edit') : __('home.claims_add') }} — {{ __('home.claims_demand_word') }}
                </h3>
            </div>
            <button type="button" @click="$wire.showDemand = false"
        ...
        <div class="bg-white dark:bg-zinc-900 px-5 py-6">
            <form wire:submit="saveDemand" class="space-y-5">
                <div class="flex flex-col gap-1">
                    ...
                    <select wire:model="demandGov" class="{{ $inp }}">
                        <option value="">— {{ __('home.claims_governorate') }} —</option>
                        @foreach($allGovernorates as $gov)
                            <option value="{{ $gov->id }}">{{ $gov->name }}</option>
                        @endforeach
                    </select>
                    @error('demandGov') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="number" wire:model="demandAmount" min="0" step="0.01" placeholder="0.00" class="{{ $inp }}" />
                    @error('demandAmount') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>
...
{{-- ── Delete Demand Modal ── --}}
<div x-show="$wire.showDeleteDemand"
     x-transition.opacity
     @click.self="$wire.showDeleteDemand = false"
     class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"
     style="display:none">
    <div x-transition:enter="transition ease-out duration-200"
         x-transition:enter-start="opacity-0 scale-95"
        ...
        <div class="flex items-center justify-between px-5 py-3.5 bg-red-500">
            ...
            <button type="button" @click="$wire.showDeleteDemand = false"
                    class="w-6 h-6 rounded-full flex items-center justify-center text-white/70 hover:text-white hover:bg-white/20 transition text-base leading-none">×</button>
        </div>
        <div class="bg-white dark:bg-zinc-900 px-5 py-6 space-y-5">
            <p class="text-sm text-zinc-600 dark:text-zinc-300 text-center">
                ...
                <span class="font-semibold text-zinc-800 dark:text-zinc-100">{{ $deletingDemandLabel }}</span>
            </p>
            <div class="flex gap-3">
                <button type="button" wire:click="deleteDemand"
