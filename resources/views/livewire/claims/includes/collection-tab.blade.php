...
<div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 space-y-4">
    ...
    <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.claims_monthly_title') }}</h3>
        </div>
    ...
    <div class="flex items-center justify-end gap-2">
        ...
        <select wire:model.live="filterYear" class="{{ $inp }} max-w-[140px]">
        ...
        <table class="w-full text-sm text-right">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                ...
                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                    <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">{{ $claim->governorate->name }}</td>
                    <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">{{ $claim->year }}</td>
                    <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">{{ $months[$claim->month] ?? $claim->month }}</td>
                    <td class="px-4 py-2.5 font-medium text-zinc-800 dark:text-zinc-100">{{ number_format($claim->value, 2) }}</td>
                    <td class="px-4 py-2.5">
                        <div class="flex items-center justify-center gap-1">
                            @if($this->canEdit())
                            <button type="button" wire:click="openEdit({{ $claim->id }})"
                            ...
                            <button type="button" wire:click="askDelete({{ $claim->id }})"
        ...
        <div class="flex items-center justify-between px-5 py-3.5 bg-[#c9a847]">
            <div class="flex items-center gap-2.5">
                ...
                <h3 class="text-sm font-semibold text-white">
                    {{ $editingId ? __('home.claims_edit') : __('home.claims_add') }} — {{ __('home.claims_monthly_title') }}
                </h3>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-900 px-5 py-6">
            <form wire:submit="save" class="space-y-5">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="number" wire:model="formValue" min="0" step="0.01" placeholder="0.00" class="{{ $inp }}" />
