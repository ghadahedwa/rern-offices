<div class="space-y-8"
     x-data
     @keydown.escape.window="$wire.showForm = false; $wire.showDelete = false; $wire.showDebt = false">
    ...
    <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 space-y-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">مديونية الطلبات</h3>
            </div>
            @if($canEdit)
            <button type="button" wire:click="openDebt"
        ...
        <div class="flex items-baseline gap-2 pr-4">
            <span class="text-2xl font-bold text-zinc-800 dark:text-zinc-100">
                {{ $office->debt_amount !== null ? number_format($office->debt_amount, 2) : '—' }}
            </span>
            @if($office->debt_amount !== null)
            <span class="text-sm text-zinc-400">جنيه</span>
            @endif
        </div>
    </div>

    {{-- ── المحصل من الطلبات (شهري) ── --}}
        ...
        <div x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-between px-5 py-3.5 bg-[#c9a847]">
                <div class="flex items-center gap-2.5">
                    ...
                    <h3 class="text-sm font-semibold text-white">تعديل مديونية الطلبات</h3>
                </div>
            ...
            <div class="bg-white dark:bg-zinc-900 px-5 py-6">
                <form wire:submit="saveDebt" class="space-y-5">
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">المبلغ (ج)</label>
