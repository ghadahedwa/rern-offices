<div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm p-6" x-data="{
    modal: null,
    fileName: '',
    uploading: false,
    viewer: null,
    ...
    <div class="space-y-8">
        ...
        <div>
            <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    ...
                    <span class="text-xs font-normal text-zinc-400 mr-1">({{ ($existingMedia['photo'] ?? collect())->count() }} / 5)</span>
                </span>
                @if(($existingMedia['photo'] ?? collect())->count() < 5)
                    <button type="button" @click="openModal('photo')"
                            class="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-[#c9a847]/10 text-[#b8962e] hover:bg-[#c9a847]/20 transition mr-auto">
                ...
                <table dir="rtl" class="w-full border-collapse">
                    ...
                    @foreach($existingMedia['photo'] as $photo)
                        <td class="py-2 pl-2 text-center" style="width:72px;">
                            ...
                            <br>
                            <button type="button" @click="askDelete({{ $photo->id }}, 'هذه الصورة')"
        ...
        <div>
            ...
            @if(($existingMedia['video'] ?? collect())->isNotEmpty())
                @foreach($existingMedia['video'] as $vid)
                    <div class="flex items-center justify-between p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50" style="max-width:250px;">
        ...
        <div>
            <div class="flex items-center gap-3 mb-3">
                <div class="w-1 h-5 bg-[#c9a847] rounded-full"></div>
                <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    {{ __('home.section_establishment_decision') }}
                ...
                @if(($existingMedia['document'] ?? collect())->count() < 1)
                    <button type="button" @click="openModal('document')"
                        ...
                        {{ __('home.add_establishment_decision') }}
                    </button>
                @endif
            </div>
            ...
            @if(($existingMedia['document'] ?? collect())->isNotEmpty())
                @foreach($existingMedia['document'] as $doc)
                    <div class="flex items-center justify-between p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50">
                        ...
                        <button type="button" @click="askDelete({{ $doc->id }}, 'قرار الإنشاء')"
                                class="text-xs text-red-500 hover:text-red-700 transition font-medium shrink-0 mr-3 cursor-pointer">حذف</button>
                    </div>
                @endforeach
            @else
        ...
        <div>
            <div class="flex items-center gap-3 mb-3">
                <div class="w-1 h-5 bg-[#c9a847] rounded-full"></div>
                <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    {{ __('home.section_license_photo') }}
                    <span class="text-xs font-normal text-zinc-400 mr-1">({{ ($existingMedia['license_photo'] ?? collect())->count() }} / 1)</span>
                </span>
                @if(($existingMedia['license_photo'] ?? collect())->count() < 1)
                    <button type="button" @click="openModal('license_photo')"
                            class="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-[#c9a847]/10 text-[#b8962e] hover:bg-[#c9a847]/20 transition mr-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                        </svg>
                        {{ __('home.add_license_photo') }}
                    </button>
                @endif
            </div>

            @if(($existingMedia['license_photo'] ?? collect())->isNotEmpty())
                @foreach($existingMedia['license_photo'] as $lic)
                    <div class="inline-flex flex-col items-center gap-1 text-center">
                        <img src="{{ asset('storage/' . $lic->path) }}"
                             alt=""
                             @click="openViewer('{{ asset('storage/' . $lic->path) }}', 'photo')"
                             style="width:64px;height:64px;object-fit:cover;"
                             class="rounded-lg border-2 border-[#c9a847] cursor-pointer transition-transform duration-150 hover:opacity-95 active:scale-90" />
                        <button type="button" @click="askDelete({{ $lic->id }}, 'صورة الرخصة')"
                                class="text-xs text-red-500 hover:text-red-700 font-medium transition whitespace-nowrap cursor-pointer">
                            حذف
                        </button>
                    </div>
            ...
            <div class="flex items-center justify-between px-5 py-3.5 bg-[#c9a847]">
                <div class="flex items-center gap-2.5">
                    ...
                    <h3 class="text-sm font-semibold text-white">
                        ...
                        <span x-show="modal === 'document'">{{ __('home.add_establishment_decision') }}</span>
                        <span x-show="modal === 'license_photo'">{{ __('home.add_license_photo') }}</span>
                    </h3>
                </div>
            ...
            <div class="bg-white dark:bg-zinc-900 px-5 py-6 space-y-5">
                <div class="rounded-xl border-8 border-dashed border-zinc-200 dark:border-zinc-700 p-5 flex flex-col items-center gap-3 text-center">
                    <div class="w-12 h-12 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
                        <svg x-show="modal === 'photo' || modal === 'license_photo'" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5M21 3.75H3A2.25 2.25 0 00.75 6v12A2.25 2.25 0 003 20.25h18A2.25 2.25 0 0023.25 18V6A2.25 2.25 0 0021 3.75z"/></svg>
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400 min-h-5">
                        <span x-show="!fileName" class="text-zinc-400">
                            ...
                            <span x-show="modal === 'license_photo'">{{ __('home.photo_types_hint') }}</span>
                        </span>
                    ...
                    <label x-show="modal === 'license_photo'" class="cursor-pointer inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-[#c9a847] text-[#b8962e] text-sm font-medium hover:bg-[#c9a847]/10 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/></svg>
                        {{ __('home.browse_files') }}
                        <input type="file" class="hidden" accept="image/*" wire:model="newLicensePhoto" @change="fileName = $event.target.files[0]?.name ?? ''" />
                    </label>
                </div>
                ...
                @error('newLicensePhoto') <p class="text-xs text-red-500 text-center -mt-2">{{ $message }}</p> @enderror
                <div class="flex gap-3">
                    <button type="button"
                            ...
                            @click="modal === 'photo' ? $wire.uploadPhoto() : (modal === 'video' ? $wire.uploadVideo() : (modal === 'document' ? $wire.uploadDocument() : $wire.uploadLicensePhoto()))">
                        <span x-show="uploading">{{ __('home.uploading') }}</span>
                        <span x-show="!uploading" wire:loading.remove wire:target="uploadPhoto,uploadVideo,uploadDocument,uploadLicensePhoto">{{ __('home.upload_file') }}</span>
                        <span x-show="!uploading" wire:loading wire:target="uploadPhoto,uploadVideo,uploadDocument,uploadLicensePhoto">{{ __('home.uploading_in_progress') }}</span>
                    </button>
