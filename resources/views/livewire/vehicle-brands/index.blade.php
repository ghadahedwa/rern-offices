<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.vehicle_brands_title') }}</h1>
        @if($isSuperAdmin)
            <a href="{{ route('vehicle-brands.create') }}" wire:navigate
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($vehicleBrands as $vehicleBrand)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-500">{{ $vehicleBrands->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100">{{ $vehicleBrand->name }}</td>
                        @if($isSuperAdmin)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('vehicle-brands.edit', $vehicleBrand) }}" wire:navigate
                                    ...
                                    <button
                                        wire:click="askDelete({{ $vehicleBrand->id }})"
