<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.vehicle_types_title') }}</h1>
        @if($isSuperAdmin)
            <a href="{{ route('vehicle-types.create') }}" wire:navigate
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($vehicleTypes as $vehicleType)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-500">{{ $vehicleTypes->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100">{{ $vehicleType->name }}</td>
                        @if($isSuperAdmin)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button
                                        wire:click="askDelete({{ $vehicleType->id }})"
