<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.work_systems') }}</h1>
        @if($isSuperAdmin)
            <a href="{{ route('work-systems.create') }}" wire:navigate
    ...
    <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm">
        <table class="w-full text-sm text-right">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($workSystems as $workSystem)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-500">{{ $workSystems->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100">{{ $workSystem->name }}</td>
                        @if($isSuperAdmin)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('work-systems.edit', $workSystem) }}" wire:navigate
                                    ...
                                    <button
                                        wire:click="askDelete({{ $workSystem->id }})"
