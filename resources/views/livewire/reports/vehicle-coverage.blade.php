<div class="max-w-7xl mx-auto p-6 space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.report_vehicle_coverage_title') }}</h1>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">{{ __('home.report_vehicle_coverage_hint') }}</p>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-center">
                <thead class="text-xs">
                    <tr class="bg-[#c9a847] text-white">
                        <th class="px-3 py-2.5 font-semibold text-right sticky right-0 bg-[#c9a847]">{{ __('home.governorate_name') }}</th>
                        <th class="px-3 py-2.5 font-semibold text-right">{{ __('home.vehicle_name') }}</th>
                        @foreach($days as $key => $label)
                            <th class="px-2 py-2.5 font-medium">{{ $label }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                    @foreach($rows as $row)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition align-top">
                            <td class="px-3 py-2.5 text-right font-medium text-zinc-700 dark:text-zinc-200 bg-[#c9a847]/5 whitespace-nowrap">{{ $row['governorate_name'] }}</td>
                            <td class="px-3 py-2.5 text-right font-medium text-zinc-800 dark:text-zinc-100 whitespace-nowrap">{{ $row['name'] }}</td>
                            @foreach($days as $key => $label)
                                @php $addr = $row['days'][$key] ?? null; @endphp
                                <td class="px-2 py-2.5 {{ $addr ? 'text-zinc-600 dark:text-zinc-300' : 'text-zinc-300 dark:text-zinc-600' }}">{{ $addr ?? '—' }}</td>
                            @endforeach
                        </tr>
                    @endforeach
                </tbody>
