<div class="max-w-5xl mx-auto p-6 space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.report_vehicle_status_title') }}</h1>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">{{ __('home.report_vehicle_status_hint') }}</p>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-center whitespace-nowrap">
                <thead class="text-xs">
                    <tr class="bg-[#c9a847] text-white">
                        <th class="px-4 py-2.5 font-semibold text-right">{{ __('home.governorate_name') }}</th>
                        @foreach($statuses as $key => $label)
                            <th class="px-3 py-2.5 font-medium">{{ $label }}</th>
                        @endforeach
                        <th class="px-3 py-2.5 font-semibold bg-[#b8962e]">{{ __('home.report_total') }}</th>
                        <th class="px-3 py-2.5 font-semibold bg-[#b8962e]">{{ __('home.report_readiness') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                    @foreach($rows as $gov)
                        @php
                            ...
                            $readiness = $rowTotal > 0 ? round($working / $rowTotal * 100) : null;
                        @endphp
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                            ...
                            @foreach($statusKeys as $st)
                                ...
                                <td class="px-3 py-2.5 font-medium {{ $v > 0 ? ($statusCellColors[$st] ?? 'text-zinc-600 dark:text-zinc-300') : 'text-zinc-300 dark:text-zinc-600' }}">{{ $v }}</td>
                            @endforeach
                            <td class="px-3 py-2.5 font-semibold text-[#b8962e] dark:text-[#c9a847] bg-[#c9a847]/5">{{ $rowTotal }}</td>
                            <td class="px-3 py-2.5 font-semibold text-zinc-700 dark:text-zinc-200">{{ $readiness !== null ? $readiness . '%' : '—' }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    @php
                        ...
                        $grandReadiness = $grandTotal > 0 ? round(($colTotals['working'] ?? 0) / $grandTotal * 100) : null;
                    @endphp
                    <tr class="bg-[#c9a847]/15 font-semibold text-zinc-800 dark:text-zinc-100">
                        ...
                        @foreach($statusKeys as $st)
                            <td class="px-3 py-3">{{ $colTotals[$st] }}</td>
                        @endforeach
                        <td class="px-3 py-3 text-[#b8962e] dark:text-[#c9a847]">{{ $grandTotal }}</td>
                        <td class="px-3 py-3">{{ $grandReadiness !== null ? $grandReadiness . '%' : '—' }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
