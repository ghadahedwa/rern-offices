<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">
            {{ $contractor?->exists ? __('home.ct_worker_edit') : __('home.ct_worker_add') }}
        </h1>
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <div>
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.ct_worker_data') }}</h3>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                            {{ __('home.ct_worker_name') }} <span class="text-red-500">*</span>
                        </label>
                    ...
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                            {{ __('home.ct_profession') }} <span class="text-red-500">*</span>
                        </label>
                        ...
                        @error('profession') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.ct_worker_phone') }}</label>
            ...
            @unless($contractor?->exists)
                {{-- التسكين الأول — يُنشأ مع الإضافة وحدها --}}
                <div>
                    <div class="flex items-center gap-3 mb-5">
                        <div class="w-1 h-5 bg-[#c9a847] rounded-full"></div>
                        <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.ct_worker_assignment') }}</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="flex flex-col gap-1">
                            <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('home.ct_worker_office') }} <span class="text-red-500">*</span>
                            </label>
                        ...
                        <div class="flex flex-col gap-1">
                            <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ __('home.ct_worker_started_on') }} <span class="text-red-500">*</span>
                            </label>
            ...
            @else
                <div class="rounded-lg border border-[#c9a847]/40 bg-[#c9a847]/[0.06] px-3.5 py-3">
                    <p class="text-xs leading-relaxed text-zinc-600 dark:text-zinc-300">{{ __('home.ct_worker_assignment_note') }}</p>
                </div>
            @endunless

            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.ct_worker_notes') }}</label>
