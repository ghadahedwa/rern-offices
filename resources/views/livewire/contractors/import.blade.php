<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.ct_import_title') }}</h1>
        <a href="{{ route('contractors.index') }}" wire:navigate
           class="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition">
            ← {{ __('home.back') }}
        </a>
    </div>

    <p class="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-4xl">{{ __('home.ct_import_hint') }}</p>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-6 space-y-5">
        <div class="flex items-center gap-3">
            ...
            <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.ct_import_step_template') }}</h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
            ...
            <div>
                <button type="button" wire:click="downloadTemplate"
                    ...
                    {{ __('home.ct_import_download') }}
                </button>
            </div>
        </div>

        <p class="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">{{ __('home.ct_import_template_note') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-6 space-y-5">
        <div class="flex items-center gap-3">
            <div class="w-1 h-5 bg-[#c9a847] rounded-full"></div>
            <h3 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('home.ct_import_step_upload') }}</h3>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.ct_import_file') }}</label>
                ...
                <p wire:loading wire:target="file" class="text-xs text-zinc-500">{{ __('home.ct_import_reading') }}</p>
                @error('file') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.ct_import_started_on') }}</label>
                <input type="date" wire:model="startedOn"
                       class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
                <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('home.ct_import_started_on_hint') }}</p>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-6 space-y-5">
            ...
            <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700">
                <table class="w-full text-sm text-right table-fixed min-w-140">
                    <thead class="bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 text-xs uppercase">
                        <tr>
                            <th class="px-4 py-3 font-medium w-[6%]">{{ __('home.ct_import_line') }}</th>
                            <th class="px-4 py-3 font-medium w-[22%]">{{ __('home.ct_worker_name') }}</th>
                            <th class="px-4 py-3 font-medium w-[14%]">{{ __('home.ct_worker_phone') }}</th>
                            <th class="px-4 py-3 font-medium w-[12%]">{{ __('home.ct_profession') }}</th>
                            <th class="px-4 py-3 font-medium w-[26%]">{{ __('home.ct_worker_office') }}</th>
                            <th class="px-4 py-3 font-medium w-[20%]">{{ __('home.ct_import_row_status') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                        @forelse($rows as $row)
                            <tr class="{{ $row['status'] === 'error' ? 'bg-red-50/60 dark:bg-red-900/10' : ($row['status'] === 'duplicate' ? 'bg-amber-50/60 dark:bg-amber-900/10' : '') }}">
                                ...
                                {{-- الصفة كما قُرئت أو كما استُنتجت — فالافتراضي يُرى قبل الحفظ --}}
                                <td class="px-4 py-2.5 text-zinc-600 dark:text-zinc-300 truncate">{{ $row['profession'] ?: '—' }}</td>
                                <td class="px-4 py-2.5 text-zinc-600 dark:text-zinc-300 truncate" title="{{ $row['office'] }}">{{ $row['office'] ?: '—' }}</td>
                                <td class="px-4 py-2.5 text-xs">
                                    @if($row['status'] === 'ok')
                                        <span class="text-green-700 dark:text-green-400">{{ __('home.ct_import_row_ok') }}</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-zinc-400">{{ __('home.ct_import_empty_file') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="flex items-center gap-3">
                <button type="button" wire:click="import" @disabled($counts['ok'] === 0)
                    ...
                    {{ __('home.ct_import_confirm', ['count' => $counts['ok']]) }}
                </button>
                <a href="{{ route('contractors.index') }}" wire:navigate
                   class="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition">
                    {{ __('home.cancel') }}
