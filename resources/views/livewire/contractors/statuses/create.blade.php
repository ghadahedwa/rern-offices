<div class="p-6 max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">
            {{ $attendanceStatus?->exists ? __('home.ct_status_edit') : __('home.ct_status_add') }}
        </h1>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-6">
        <form wire:submit="save" class="space-y-5">
            ...
            <div class="flex flex-col gap-2">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.ct_status_color') }}</label>
            ...
            @if($attendanceStatus?->is_system)
                <div class="rounded-lg border border-[#c9a847]/40 bg-[#c9a847]/[0.06] px-3.5 py-3">
                    <p class="text-xs leading-relaxed text-zinc-600 dark:text-zinc-300">{{ __('home.ct_status_system_note') }}</p>
                </div>
            @else
                <label class="flex items-start gap-3 cursor-pointer">
                    ...
                    <span>
                        ...
                        <span class="block text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{{ __('home.ct_status_active_hint') }}</span>
                    </span>
                </label>
            @endif
