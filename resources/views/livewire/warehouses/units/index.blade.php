<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.item_units_title') }}</h1>
        @if($canManage)
            <a href="{{ route('item-units.create') }}" wire:navigate
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($units as $unit)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-500">{{ $units->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100">{{ $unit->name }}</td>
                        @if($canManage)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('item-units.edit', $unit) }}" wire:navigate
                ...
                @empty
                    <tr>
                        <td colspan="{{ $canManage ? 3 : 2 }}" class="px-4 py-10 text-center text-zinc-400">
                            {{ __('home.no_data') }}
                        </td>
                    </tr>
                @endforelse
