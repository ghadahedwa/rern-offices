<div class="p-6 max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.wh_transfer_add') }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        {{ __('home.wh_from_warehouse') }} <span class="text-red-500">*</span>
                    </label>
                    <select wire:model.live="from_warehouse_id"
                        ...
                        <option value="">— {{ __('home.wh_from_warehouse') }} —</option>
                        {{-- المصدر من نطاق المستخدم وحده — الخصم يقع عليه.
                             والوجهة تبقى كل المخازن: أمين الرئيسي ينقل إلى المحافظات كلها. --}}
                        @foreach($sourceWarehouses as $wh)
                            <option value="{{ $wh->id }}">{{ $wh->name }} @if($wh->type) ({{ $wh->type->name }}) @endif</option>
                        @endforeach
                    </select>
                    @error('from_warehouse_id') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>

                {{-- المخزن المستلم --}}
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        {{ __('home.wh_to_warehouse') }} <span class="text-red-500">*</span>
                    </label>
                    <select wire:model="to_warehouse_id"
                            class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]">
                        <option value="">— {{ __('home.wh_to_warehouse') }} —</option>
                        @foreach($warehouses as $wh)
                            <option value="{{ $wh->id }}">{{ $wh->name }} @if($wh->type) ({{ $wh->type->name }}) @endif</option>
                        @endforeach
                    </select>
                    @error('to_warehouse_id') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>

                {{-- تاريخ الصرف --}}
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        {{ __('home.wh_transferred_at') }} <span class="text-red-500">*</span>
                    </label>
                    <input type="date" wire:model="transferred_at"
                           class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
                    @error('transferred_at') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>

                {{-- نوع المستند --}}
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.wh_document_type') }}</label>
                    <input type="text" wire:model="document_type" list="wh-document-types"
                           class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
                    <datalist id="wh-document-types">
                    ...
                    @foreach($lines as $i => $line)
                        <div class="flex flex-wrap items-start gap-3" wire:key="line-{{ $i }}">
                            ...
                            <div class="flex-1 flex flex-col gap-1">
                                ...
                                @if(($line['item_id'] ?? '') !== '')
                                    <p class="text-xs text-zinc-400 dark:text-zinc-500">{{ __('home.wh_current_balance') }}: {{ $stocks[(int) $line['item_id']] ?? 0 }}</p>
                                @endif
