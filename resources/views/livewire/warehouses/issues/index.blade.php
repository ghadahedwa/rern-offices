<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.wh_issues') }}</h1>
        @if($canCreate)
            <a href="{{ route('warehouses.issues.create') }}" wire:navigate
    ...
    <x-filter-bar :active="$this->hasActiveFilters()" :per-page-options="$this->perPageOptions()" :columns="4">
        <x-filter-input :label="__('home.search')" wire:model.live.debounce.300ms="search"
                        placeholder="{{ __('home.wh_issue_office') }} / {{ __('home.warehouse') }} / {{ __('home.wh_document_type') }}" />
    ...
    <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm">
        <table class="w-full text-sm text-right">
            <thead class="bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 text-xs uppercase">
                <tr>
                    @include('livewire.partials.sortable-th', ['column' => 'issued_at',     'label' => __('home.wh_issue_at')])
                    @include('livewire.partials.sortable-th', ['column' => 'warehouse',     'label' => __('home.wh_issue_warehouse')])
                    @include('livewire.partials.sortable-th', ['column' => 'office',        'label' => __('home.wh_issue_office')])
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($issues as $issue)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-600 dark:text-zinc-300">{{ $issue->issued_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-3 text-zinc-700 dark:text-zinc-200">{{ $issue->warehouse?->name ?? '—' }}</td>
                        {{-- المقر هو جواب «أين ذهب» — فهو العمود الأبرز --}}
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100 max-w-64 truncate"
                            title="{{ $issue->office?->name }}">{{ $issue->office?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-zinc-600 dark:text-zinc-300">{{ $issue->document_type ?: '—' }}</td>
                        <td class="px-4 py-3 text-zinc-600 dark:text-zinc-300">{{ $issue->items_count }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <button wire:click="view({{ $issue->id }})"
                                ...
                                @if($canDelete)
                                    <button wire:click="askDelete({{ $issue->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-zinc-400">{{ __('home.wh_no_issues') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="$wire.showView"
        ...
        <div x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-between px-5 py-3.5 bg-zinc-100 dark:bg-zinc-800">
                <h3 class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{{ __('home.wh_issues') }}</h3>
            ...
            @if($viewing)
                <div class="bg-white dark:bg-zinc-900 px-5 py-5 space-y-4">
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        <div>
                            <p class="text-xs text-zinc-400 dark:text-zinc-500 mb-0.5">{{ __('home.wh_issue_warehouse') }}</p>
                            <p class="text-zinc-800 dark:text-zinc-100">{{ $viewing->warehouse?->name ?? '—' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-zinc-400 dark:text-zinc-500 mb-0.5">{{ __('home.wh_issue_office') }}</p>
                            <p class="text-zinc-800 dark:text-zinc-100">{{ $viewing->office?->name ?? '—' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-zinc-400 dark:text-zinc-500 mb-0.5">{{ __('home.wh_issue_at') }}</p>
                            <p class="text-zinc-800 dark:text-zinc-100">{{ $viewing->issued_at->format('Y-m-d') }}</p>
                        </div>
                        <div>
