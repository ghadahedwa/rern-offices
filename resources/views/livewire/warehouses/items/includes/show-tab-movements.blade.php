{{-- تاب «سجل الحركات»: حركات هذا الصنف في المخازن كلها --}}
...
<x-filter-bar :active="$this->hasActiveFilters()" :per-page-options="$this->perPageOptions()" :columns="5">
    ...
    <x-filter-select :label="__('home.warehouse')" wire:model.live="warehouseFilter">
        ...
        @foreach($movementWarehouses as $wh)
        ...
        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
            @forelse($movements as $movement)
                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                    ...
                    <td class="px-4 py-3 text-zinc-600 dark:text-zinc-300 tabular-nums">{{ number_format($movement->quantity) }}</td>
                    <td class="px-4 py-3 text-zinc-400 dark:text-zinc-500 tabular-nums">{{ number_format($movement->balance_before) }}</td>
                    <td class="px-4 py-3 text-zinc-600 dark:text-zinc-300 tabular-nums">{{ number_format($movement->balance_after) }}</td>
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-4 py-10 text-center text-zinc-400">
                        {{ $this->hasActiveFilters() ? __('home.wh_item_no_movements_match') : __('home.wh_item_no_movements') }}
                    </td>
                </tr>
            @endforelse
        </tbody>
