<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.vehicle_device_types_title') }}</h1>
        @if($isSuperAdmin)
            <a href="{{ route('vehicle-device-types.create') }}" wire:navigate
               class="inline-flex items-center gap-2 bg-[#c9a847] hover:bg-[#b8962e] text-white text-sm font-medium px-4 py-2 rounded-lg transition">
                ...
                {{ __('home.add_vehicle_device_type') }}
            </a>
        @endif
    </div>
    ...
    <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm">
        <table class="w-full text-sm text-right">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                @forelse($vehicleDeviceTypes as $vehicleDeviceType)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-3 text-zinc-500">{{ $vehicleDeviceTypes->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-zinc-100">{{ $vehicleDeviceType->name }}</td>
                        @if($isSuperAdmin)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('vehicle-device-types.edit', $vehicleDeviceType) }}" wire:navigate
                                    ...
                                    <button
                                        wire:click="askDelete({{ $vehicleDeviceType->id }})"
    ...
    <div>{{ $vehicleDeviceTypes->links() }}</div>
