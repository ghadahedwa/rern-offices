<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.ct_holidays') }}</h1>
        <div class="flex items-center gap-2">
            <button type="button" wire:click="seedFixed" title="{{ __('home.ct_holiday_seed_hint') }}"
            ...
            <a href="{{ route('official-holidays.create') }}" wire:navigate
               class="inline-flex items-center gap-2 bg-[#c9a847] hover:bg-[#b8962e] text-white text-sm font-medium px-4 py-2 rounded-lg transition">
                ...
                {{ __('home.ct_holiday_add') }}
            </a>
        </div>
    </div>

    <p class="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-3xl">{{ __('home.ct_holidays_hint') }}</p>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <select wire:model.live="year"
            ...
            <option value="">{{ __('home.ct_holiday_all_years') }}</option>
            @foreach($years as $y)
        ...
        <table class="w-full text-sm text-right">
            <thead class="bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 text-xs uppercase">
                <tr>
                    ...
                    <th class="px-4 py-3 font-medium">{{ __('home.ct_holiday_name') }}</th>
                    <th class="px-4 py-3 font-medium">{{ __('home.ct_holiday_starts_on') }}</th>
                    <th class="px-4 py-3 font-medium">{{ __('home.ct_holiday_ends_on') }}</th>
                    <th class="px-4 py-3 font-medium">{{ __('home.ct_holiday_days_count') }}</th>
    ...
    <div class="rounded-lg border border-[#c9a847]/40 bg-[#c9a847]/[0.06] px-3.5 py-3 max-w-3xl">
        <p class="text-xs leading-relaxed text-zinc-600 dark:text-zinc-300">{{ __('home.ct_holiday_super_admin_note') }}</p>
    </div>
