        ...
        <form wire:submit="save" class="space-y-5">
            ...
            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    {{ __('home.governorate_name') }} <span class="text-red-500">*</span>
                </label>
            ...
            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    {{ __('home.order') }} <span class="text-red-500">*</span>
                </label>
                <input type="number" wire:model="order" min="0" autocomplete="off"
                       class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
            ...
            {{-- latitude / longitude — hidden for now
            <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-1">
                    ...
                    <input type="number" step="any" wire:model="latitude"
                           class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
                    @error('latitude') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('home.longitude') }}</label>
                    <input type="number" step="any" wire:model="longitude"
                           class="border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-sm w-full bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#c9a847]" />
                    @error('longitude') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
                </div>
            </div>
            --}}
