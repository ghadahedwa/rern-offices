<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-zinc-800 dark:text-zinc-100">{{ __('home.location_description') }}</h1>
        @if($isSuperAdmin)
            <a href="{{ route('location-descriptions.create') }}" wire:navigate
               class="inline-flex items-center gap-2 bg-[#c9a847] hover:bg-[#b8962e] text-white text-sm font-medium px-4 py-2 rounded-lg transition">
                ...
                {{ __('home.add_location_description') }}
            </a>
        @endif
    </div>
                        ...
                        @if($isSuperAdmin)
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('location-descriptions.edit', $item) }}" wire:navigate
