<div class="max-w-4xl mx-auto p-6 space-y-6">
    ...
    {{-- Empty state (الموديول قيد الإنشاء) --}}
    <div class="rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-12 flex flex-col items-center justify-center text-center gap-3">
        <div class="w-16 h-16 rounded-full bg-[#c9a847]/10 flex items-center justify-center">
            <flux:icon.calendar-days class="w-8 h-8 text-[#c9a847]" />
        </div>
        <p class="text-lg font-semibold text-zinc-700 dark:text-zinc-200">{{ __('home.meetings_empty') }}</p>
        <p class="text-sm text-zinc-500 dark:text-zinc-400 max-w-md">{{ __('home.meetings_empty_hint') }}</p>
    </div>
