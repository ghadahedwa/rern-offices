...
class VehicleDeviceCountPdfController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $f = session('vehicle_device_count_filters', []);

        $isSuperAdmin  = $user->hasRole('super-admin');
        $allowedGovIds = $isSuperAdmin ? null : $user->governorates()->pluck('governorates.id')->all();

        $m = VehicleDeviceCountMatrix::build($allowedGovIds, $f);
        ...
        $html = view('print.vehicle-device-count-pdf', compact(
        ...
        $filename = 'vehicle-device-count-' . now()->format('Ymd-His') . '.pdf';
