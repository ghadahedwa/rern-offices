...
class Law9Registrations extends Component
{
    use WithPagination;
    ...
    public function mount(Office $office): void
    {
        ...
        foreach (StatType::where('group_key', 'law9_registrations')->pluck('id') as $id) {
            $this->filterYear[$id]  = '';
            $this->filterMonth[$id] = '';
        }
    }
    ...
    {
        $statTypes = StatType::where('group_key', 'law9_registrations')
            ->orderBy('order')
            ->get();
        ...
        return view('livewire.offices.stat-tab.law9-registrations', [
            'statTypes' => $statTypes,
            'existing'  => $existing,
            'years'     => $years,
        ]);
