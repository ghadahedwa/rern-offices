...
class RegistryFormsAndFolders extends Component
{
    use WithPagination;
    ...
    public function mount(Office $office): void
    {
        ...
        foreach (StatType::where('group_key', 'registry_forms_folders')->pluck('id') as $id) {
            $this->filterYear[$id]  = '';
            $this->filterMonth[$id] = '';
        }
    }
    ...
    {
        $statTypes = StatType::where('group_key', 'registry_forms_folders')
            ->orderBy('order')
            ->get();
        ...
        return view('livewire.offices.stat-tab.registry-forms-and-folders', [
            'statTypes' => $statTypes,
            'existing'  => $existing,
            'years'     => $years,
            'months'    => $months,
