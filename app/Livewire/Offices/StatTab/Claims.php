...
class Claims extends Component
{
    ...
    public string $formValueType = 'amount';
    ...
    {
        $this->office = $office;
        $this->debtAmount = $office->debt_amount !== null ? (string) (0 + $office->debt_amount) : '';

        foreach (StatType::where('group_key', 'claims')->pluck('id') as $id) {
            $this->filterYear[$id]  = '';
            $this->filterMonth[$id] = '';
        }
    }
    ...
    public function saveDebt(): void
    {
        ...
        $this->office->update(['debt_amount' => $this->debtAmount !== '' ? $this->debtAmount : null]);

        $this->showDebt = false;
        Flux::toast(variant: 'success', text: 'تم حفظ المديونية');
    }
    ...
    {
        $statTypes = StatType::where('group_key', 'claims')
            ->orderBy('order')
            ->get();
        ...
        return view('livewire.offices.stat-tab.claims', [
            'statTypes' => $statTypes,
            'existing'  => $existing,
            'years'     => $years,
            'months'    => $months,
