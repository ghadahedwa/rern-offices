...
class Index extends Component
{
    ...
    public function askDelete(int $id): void
    {
        ...
        $this->deletingId    = $vehicleBrand->id;
        $this->deletingLabel = $vehicleBrand->name;
    ...
    public function deleteRow(): void
    {
        abort_unless(Auth::user()?->can('offices.settings'), 403);
        if ($this->deletingId) {
            VehicleBrand::findOrFail($this->deletingId)->delete();
            $this->reset('deletingId', 'deletingLabel', 'showDelete');
            Flux::toast(variant: 'success', text: __('home.vehicle_brand_deleted'));
        }
    }

    public function render()
    {
        return view('livewire.vehicle-brands.index', [
