...
class VehiclesExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithTitle, ShouldAutoSize
{
     ...
     */
    public function __construct(protected Collection $vehicles, ?array $columnKeys = null)
    {
        $this->columns = VehicleColumns::select($columnKeys);
    }
    ...
    {
        return array_map(fn ($c) => $c['value']($vehicle), array_values($this->columns));
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // تجميد أول عمودين (المحافظة + الاسم) + صفّي الرؤوس + فلتر تلقائي
                $sheet->freezePane('C3');
