...
function holidayContractor(): Contractor
{
...
function absenceOn(Contractor $contractor, string $date): AttendanceDay
{
...
it('يمنع مديرَ القوائم المرجعية من شاشة العطلات', function () {
    // ⚠️ أضيق من contractors.settings عمداً: القائمة قومية لا محلية
    $this->actingAs(holidayUser(['contractors.settings']));
...
it('يمنع صاحب صلاحيات الفرع التشغيلية', function () {
    $this->actingAs(holidayUser(['contractors.index', 'contractors.attendance', 'contractors.export']));
...
it('لا يحذف حذفُ العطلة سجلات الحضور من جدولها', function () {
    ...
    $contractor = holidayContractor();
    absenceOn($contractor, '2026-09-16');
...
it('لا يحفظ العطلةَ فوراً إذا وقع داخلها غياب مسجَّل', function () {
    ...
    $contractor = holidayContractor();
    absenceOn($contractor, '2026-09-17');
...
it('يحذف السجلات المتعارضة ويعتمد العطلة بعد الموافقة', function () {
    ...
    $contractor = holidayContractor();
    absenceOn($contractor, '2026-09-17');
    absenceOn($contractor, '2026-09-16');   // خارج مدى العطلة — لا يُمسّ
...
it('يحفظ بلا مودال حين لا تعارض', function () {
    ...
    $contractor = holidayContractor();
    absenceOn($contractor, '2026-09-10');
