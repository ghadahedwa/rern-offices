...
function contractorServing(string $from, ?string $to = null, ?Office $office = null): Contractor
{
...
it('يبدأ حساب المدخل من تاريخ التحاقه لا من أول المدى', function () {
    ...
    $contractor = contractorServing('2026-09-15');

    expect(WorkingDays::contractorCalendar($contractor, SEP_FROM, SEP_TO))->toHaveCount(14);
});

it('يقف حساب المدخل عند انتهاء خدمته', function () {
    $contractor = contractorServing('2026-09-01', '2026-09-10');

    // ١ إلى ١٠ بلا الجمعتين ٤ و١١ (١١ خارج المدة أصلاً) = ٩
    expect(WorkingDays::contractorCalendar($contractor, SEP_FROM, SEP_TO))->toHaveCount(9);
});

it('لا يحسب يوماً لمدخل بلا تسكين', function () {
    $contractor = Contractor::factory()->create();

    expect(WorkingDays::contractorCalendar($contractor, SEP_FROM, SEP_TO))->toBe([]);
});

it('يحسب المنقول بين مقرّين مرة واحدة لا مرتين', function () {
    $contractor = contractorServing('2026-09-01', '2026-09-15');
    ...
    $days = WorkingDays::contractorCalendar($contractor->fresh(), SEP_FROM, SEP_TO);
...
it('يكشف تداخل التسكينات ولا يعترض على تتابعها', function () {
    ...
    $following = new ContractorAssignment([
        'contractor_id' => $contractor->id,
        'office_id'   => $office->id,
        'started_on'  => '2026-09-16',
    ]);

    $overlapping = new ContractorAssignment([
        'contractor_id' => $contractor->id,
        'office_id'   => $office->id,
...
it('يشتق الحضور من أيام العمل ناقص الغياب والإجازات', function () {
    $contractor = contractorServing('2026-09-01');
    ...
    $summary = WorkingDays::summaryFor($contractor, SEP_FROM, SEP_TO);
...
it('يكشف تداخلاً مع تسكينٍ مفتوح مهما بعُد تاريخ الجديد', function () {
    ...
    $later = new ContractorAssignment([
        'contractor_id' => $contractor->id,
        'office_id'   => Office::factory()->create()->id,
