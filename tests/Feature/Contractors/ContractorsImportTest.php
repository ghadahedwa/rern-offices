...
use App\Livewire\Contractors\Import;
...
{
    $path = tempnam(sys_get_temp_dir(), 'ct_test_').$suffix;
...
{
    // ⚠️ مباشرةً لا عبر ContractorScope: النطاق يقرأ المستخدم المسجَّل، وفي اختبارات
...
{
    $template = new ContractorsTemplate($governorate, governorateOffices($governorate));
    ...
    foreach ($rows as $row) {
        // الصفة اختيارية في الاستدعاء: [اسم، هاتف، مقر] أو [اسم، هاتف، مقر، صفة]
        [$name, $phone, $office] = $row;
        $profession = $row[3] ?? '';

        $sheet->setCellValue('A'.$line, $name);
        $sheet->setCellValueExplicit('B'.$line, $phone, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        $sheet->setCellValue('C'.$line, $profession);
        $sheet->setCellValue('D'.$line, $office);
        $line++;
    }
    ...
    return UploadedFile::fake()->createWithContent('contractors.xlsx', file_get_contents($path));
}
...
it('يبني القالب بمقرات المحافظة وحدها', function () {
    ...
    $this->actingAs(importUser(['contractors.index', 'contractors.create'], [$gov]));

    $path = (new ContractorsTemplate($gov, governorateOffices($gov)))->saveTo(importTempPath());
...
it('يجعل عمود التليفون نصّياً فلا يسقط الصفر الأول', function () {
    ...
    expect($sheet->getStyle('B2')->getNumberFormat()->getFormatCode())->toBe(NumberFormat::FORMAT_TEXT)
        ->and($sheet->getStyle('B'.(ContractorsTemplate::ROWS + 1))->getNumberFormat()->getFormatCode())
...
it('يضع قائمة منسدلة للمقرات على عمود المقر', function () {
    ...
    // المقر في العمود D بعد إضافة عمود الصفة، والصفة في C — لكلٍّ نطاقه المسمّى
    expect($collection)->toHaveKey('D2')
        ->and($collection)->toHaveKey('D'.(ContractorsTemplate::ROWS + 1))
        ->and($collection['D2']->getType())->toBe(\PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_LIST)
        ->and($collection['D2']->getFormula1())->toContain('OfficeList')
        ->and($collection)->toHaveKey('C2')
        ->and($collection['C2']->getFormula1())->toContain('ProfessionList');

    // والنطاقان المسمّيان يشيران لورقة المراجع المخفية
    $named = collect(IOFactory::load($path)->getNamedRanges());

    // ⚠️ PhpSpreadsheet يفهرس النطاقات المسمّاة بأحرف كبيرة عند القراءة
    expect($named->keys()->all())->toEqualCanonicalizing(['OFFICELIST', 'PROFESSIONLIST'])
        ->and($named->get('OFFICELIST')->getValue())->toContain('المقرات')
        ->and($named->get('PROFESSIONLIST')->getValue())->toContain('المقرات');
});

it('يقبل قالباً قديماً بلا عمود صفة ويقرأ الصفة الافتراضية', function () {
    // ⚠️ الأعمدة تُقرأ بأسماء رؤوسها لا بمواضعها: قالبٌ قديم مملوء عند المفتش كان
    //    سيصير كل صفوفه «المقر مطلوب» لأن إضافة عمود الصفة أزاحت المقر من C إلى D.
    $gov    = Governorate::factory()->create();
    $office = Office::factory()->create(['governorate_id' => $gov->id, 'name' => 'مقر التجربة']);

    $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
    $sheet       = $spreadsheet->getActiveSheet();

    // قالب ثلاثة أعمدة بالرؤوس القديمة
    $sheet->fromArray([
        ['اسم مدخل البيانات', 'رقم التليفون', 'المقر'],
        ['سعيد القديم', '01011112222', 'مقر التجربة'],
    ], null, 'A1');
    ...
    $rows = (new App\Support\Contractors\ContractorsImport($gov, governorateOffices($gov)))->parse($path);

    expect($rows)->toHaveCount(1)
        ->and($rows[0]['status'])->toBe(App\Support\Contractors\ContractorsImport::STATUS_OK)
        ->and($rows[0]['office_id'])->toBe($office->id)
        ->and($rows[0]['profession'])->toBe('مدخل بيانات');
});

it('يرفض صفاً بصفةٍ غير معروفة ولا يخترعها', function () {
    $gov = Governorate::factory()->create();
    Office::factory()->create(['governorate_id' => $gov->id, 'name' => 'مقر التجربة']);

    $file = filledTemplate($gov, [['هالة سمير', '01011112222', 'مقر التجربة', 'طبيب']]);
    $rows = (new App\Support\Contractors\ContractorsImport($gov, governorateOffices($gov)))->parse($file->getRealPath());

    expect($rows)->toHaveCount(1)
        ->and($rows[0]['status'])->toBe(App\Support\Contractors\ContractorsImport::STATUS_ERROR)
        ->and($rows[0]['profession_id'])->toBeNull()
        ->and(App\Models\Profession::where('name', 'طبيب')->exists())->toBeFalse();
});

it('يحفظ صفة الصفّ كما اختيرت في الملف', function () {
    $gov    = Governorate::factory()->create();
    $office = Office::factory()->create(['governorate_id' => $gov->id, 'name' => 'مقر التجربة']);
    ...
    Livewire::test(Import::class)
        ...
        ->set('startedOn', '2026-09-01')
        ->set('file', filledTemplate($gov, [
            ['مترجم الفرع', '01011112222', 'مقر التجربة', 'مترجم'],
            ['عامل الفرع', '01033334444', 'مقر التجربة'],
        ]))
        ->call('import');

    expect(Contractor::firstWhere('name', 'مترجم الفرع')->profession->name)->toBe('مترجم')
        ->and(Contractor::firstWhere('name', 'عامل الفرع')->profession->name)->toBe('مدخل بيانات');
});
...
it('ينزّل القالب لصاحب الصلاحية ويمنعه عن غيره وعن محافظةٍ خارج نطاقه', function () {
    ...
    $this->actingAs(importUser(['contractors.index'], [$mine]));
    $this->get(route('contractors.import'))->assertForbidden();

    $this->actingAs(importUser(['contractors.index', 'contractors.create'], [$mine]));
    $this->get(route('contractors.import'))->assertOk();
...
it('يسكّن الصفوف الصحيحة ويحفظ صفر الهاتف', function () {
    ...
    $contractor = Contractor::firstWhere('name', 'أحمد سعيد');

    expect(Contractor::count())->toBe(2)
        ->and($contractor->phone)->toBe('01012345678')
        ->and($contractor->currentAssignment->office_id)->toBe($office->id)
        ->and($contractor->currentAssignment->started_on->toDateString())->toBe('2026-09-15');
});
...
it('يحوّل الأرقام العربية في الهاتف', function () {
    ...
    expect(Contractor::first()->phone)->toBe('01098765432');
});
...
it('يرفض صفاً بمقرٍّ ليس في المحافظة أو بلا اسم أو بهاتف خاطئ', function () {
    ...
    expect(Contractor::count())->toBe(1)
        ->and(Contractor::first()->name)->toBe('نادر');
});
...
it('يتجاوز المسجَّل بالفعل في المقر نفسه', function () {
    ...
    $existing = Contractor::factory()->create(['name' => 'أحمد سعيد', 'phone' => '01012345678']);
...
it('يمنع الاستيراد عمّن سُحبت صلاحيته والشاشة مفتوحة', function () {
    ...
    $user = importUser(['contractors.index', 'contractors.create'], [$gov]);
    $this->actingAs($user);
...
it('يعبّئ القالب عمودَ الصفة بالافتراضية ولا تصير صفوفه الفارغة أخطاءً', function () {
    // ⚠️ الصفّ الفارغ يُقاس بالاسم والهاتف والمقر لا بالصفة — وإلا صار القالب
    //    المعبَّأ سلفاً ثلاثمئة خطأ «الاسم مطلوب» في ملفٍ سليم.
    $gov = Governorate::factory()->create();
    Office::factory()->create(['governorate_id' => $gov->id, 'name' => 'مقر التجربة']);
    ...
    expect($sheet->getCell('C2')->getValue())->toBe('مدخل بيانات')
        ->and($sheet->getCell('C'.(ContractorsTemplate::ROWS + 1))->getValue())->toBe('مدخل بيانات');

    // ملفٌ نزل ولم يُملأ: صفوفه كلها بلا اسم — فلا صفّ يُقرأ أصلاً
    expect((new App\Support\Contractors\ContractorsImport($gov, governorateOffices($gov)))->parse($path))
        ->toBeEmpty();
});

it('يقرأ الصفة الأساسية افتراضاً ولو تغيّر ترتيب العرض', function () {
    // ⚠️ الافتراضية تُعرَف بـis_system لا بأول الترتيب: المدير يعدّل الترتيب من
    //    شاشة الصفات، فلو صار «مترجم» أولاً لصارت ملفات المفتشين تُستورد مترجمين صامتةً.
    $gov = Governorate::factory()->create();
    Office::factory()->create(['governorate_id' => $gov->id, 'name' => 'مقر التجربة']);

    App\Models\Profession::where('name', 'مترجم')->update(['order' => 0]);

    $file = filledTemplate($gov, [['سعيد بلا صفة', '01011112222', 'مقر التجربة', '']]);
    $rows = (new App\Support\Contractors\ContractorsImport($gov, governorateOffices($gov)))->parse($file->getRealPath());

    expect($rows[0]['profession'])->toBe('مدخل بيانات');
});
