...
it('يُخفي الصنف المختار في صفٍّ آخر ويُبقيه في صفّه', function () {
    ...
    // ⚠️ يُعدّ **نصّ الخيار** لا `value=` — منسدلة حصر القسم تحمل معرّفات
    //    أقسامٍ قد تساوي معرّفات أصناف، فالعدّ بالمعرّف صار ملتبساً.
    expect(substr_count($html, '>جهاز كمبيوتر</option>'))->toBe(1);
});
